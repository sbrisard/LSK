...
\begin{equation}
  ℰ_{,u}[u^\ast(λ) + ξ_i v_i + w, λ; U] = 0
\end{equation}
or, equivalently
\begin{equation}
  \label{eq:20220901120544}
  ℰ_{,u}[u^\ast(λ) + ξ_i v_i + w, λ; V] = 0
\end{equation}
and
\begin{equation}
  \label{eq:20220825143616}
  ℰ_{,u}[u^\ast(λ) + ξ_i v_i + w, λ; W] = 0.
\end{equation}
These two equations are analysed separately in Secs.~\ref{sec:20220901145441}
and \ref{sec:20220901145523}.

\subsection{Analysis of Eq.~\eqref{eq:20220825143616}: \(w\) as a function of \(ξ_i\) and \(λ\)}
\label{sec:20220901145441}

From the implicit function theorem, Eq.~\eqref{eq:20220825143616} defines a
function \((ξ_1, \ldots, ξ_m, λ) \mapsto w(ξ_1, \ldots, ξ_m, λ)\) in the
neighborhood of \((ξ₁, \ldots, ξ_m, λ) = (0, \ldots, 0, λ₀)\). Why the theorem
applies will be clarified below. Eq.~\eqref{eq:20220825143616} is first
differentiated \wrt{} \(ξ_i\)
\begin{equation}
  \label{eq:20220826140926}
  ℰ_{,uu}(u^\ast + ξ_k v_k + w, λ; v_i + w_{,i}, W) = 0.
\end{equation}
...
\label{eq:20220825150219}
  ℰ₂(λ₀; v_i + w_{,i}, W) = ℰ₂(λ₀; w_{,i}, W) = 0.
\end{equation}

Since \(w ∈ W\) for all \(ξ^i\) and \(λ\), we have \(w_{,i} ∈ W\) and, from
Eq.~\eqref{eq:20220825150219}, \(ℰ₂(λ₀; w_{,i}, w_{,i}) = 0\), which shows that
the derivatives of \(w\) \wrt{} the \(ξ^i\) vanish at the point
\(ξ^1 = 0, \ldots, ξ^m = 0\) and \(λ = λ₀\). Eq.~\eqref{eq:20220825143616} is
then differentiated \wrt{} \(λ\)
\begin{equation}
  \label{eq:20220830145945}
  ℰ_{,uu}(u^\ast + ξ_i v_i + w, λ; \dot{u}^\ast + w_{,λ}, W) + ℰ_{,uλ}(u^\ast + ξ_i v_i + w, λ; W) = 0
\end{equation}
and, at \(ξ₁ = \ldots = ξ_m = 0\)
\begin{equation}
  \label{eq:20220830151513}
  ℰ_{,uu}(u^\ast, λ; w_{,λ}, W)
  + \underbrace{ℰ_{,uu}(u^\ast, λ; \dot{u}^\ast, W) + ℰ_{,uλ}(u^\ast, λ; W)}_{=0 \quad \text{see Eq.~\eqref{eq:20220901143843}}}
  = ℰ₂(λ; w_{,λ}, W) = 0,
\end{equation}
which proves similarly that the derivative of \(w\) \wrt{} \(λ\) vanishes at the
critical point. We have found so far that
\begin{equation}
...
differentiated first \wrt{} \(ξ_j\), then \wrt{} \(λ\). This delivers,
\begin{equation}
  ℰ_{,uuu}(u^\ast + ξ_k v_k + w, λ; v_i + w_{,i}, v_j + w_{,j}, W) + ℰ_{,uu}(u^\ast + ξ_k v_k + w, λ; w_{,ij}, W) = 0
\end{equation}
and
\begin{equation}
  \begin{aligned}[b]
    ℰ_{,uuu}(u^\ast + ξ_k v_k + w, λ; v_i + w_{,i}, \dot{u}^\ast + w_{,λ}, W) &\\
    + ℰ_{,uuλ}(u^\ast + ξ_k v_k + w, λ; v_i + w_{,i}, W) + ℰ_{,uu}(u^\ast + ξ_k v_k + w, λ; w_{,iλ}, W) &= 0
  \end{aligned}
\end{equation}
and, at \(ξ_1 = \cdots = ξ_m = 0, λ = λ₀\) (recalling that, at this point,
\(w_{,1} = \cdots = w_{, m} = w_{,λ} = 0\))
\begin{equation}
  ℰ_3(λ₀; v_i, v_j, W) + ℰ₂(λ₀; w_{,ij}, W) = 0
  \quad \text{and} \quad
  \dot{ℰ}₂(λ₀; v_i, W) + ℰ₂(λ₀; w_{,iλ}, W) = 0.
\end{equation}
...
\subsection{Analysis of Eq.~\eqref{eq:20220901120544}: \(λ\) as a function of \(ξ_i\)}
\label{sec:20220901145523}

We now turn to Eq.~\eqref{eq:20220901120544}. Since \(w\) is a function of \(λ\)
and \(ξ_k\) (\(k = 1, \ldots, m\)) this equation implicitly defines \(λ\) as a
function of \(ξ_i\). Unless otherwise mentioned, the differentials of the energy
\(ℰ_{,uu}\), \(ℰ_{,uλ}\), \(ℰ_{,λλ}\), \(ℰ_{,uuu}\) \dots{} are evaluated at
\(u = u^\ast(λ) + ξ_k v_k + w(ξ_k, λ)\). Differentiating first \wrt{} \(ξ_i\)
\begin{equation}
  \label{eq:20220901121940}
  ℰ_{,uu}[v_i + w_{,i} + λ_{,i} (\dot{u}^\ast + w_{,λ}), V] + λ_{, i} ℰ_{,uλ}(V) = 0,
\end{equation}
then \wrt{} \(ξ_j\)
\begin{equation}
  \label{eq:20220901125230}
  \begin{gathered}[b]
    ℰ_{,uuu}[v_i + w_{,i} + λ_{,i} (\dot{u}^\ast + w_{,λ}), v_j + w_{,j} + λ_{,j} (\dot{u}^\ast + w_{,λ}), V]\\
    + λ_{,j}ℰ_{,uuλ}[v_i + w_{,i} + λ_{,i} (\dot{u}^\ast + w_{,λ}), V]\\
    + ℰ_{,uu}[w_{,ij} + λ_{,ij} (\dot{u}^\ast + w_{,λ}) + λ_{,i}λ_{,j} (\ddot{u}^\ast + w_{,λλ}), V]\\
    + λ_{, ij} ℰ_{,uλ}(V) + λ_{, i} ℰ_{,uuλ}[v_j + w_{,j} + λ_{,j} (\dot{u}^\ast + w_{,λ}), V] + λ_{,i} λ_{,j} ℰ_{,uλλ}(V)= 0,
  \end{gathered}
\end{equation}

Plugging \(ξ₁ = \cdots = ξ_m = 0\) in Eqs.~\eqref{eq:20220901121940} and
\eqref{eq:20220901125230}
\begin{equation}
  \label{eq:20220901152056}
  ℰ_{,uu}(u₀, λ₀; v_i, V) + λ_{, i} \bigl\{ ℰ_{,uu}[u₀, λ₀; \dot{u}^\ast(λ₀), V] +  ℰ_{,uλ}(u₀, λ₀; V) \bigr\} = 0,
\end{equation}
% \begin{equation}
%   \begin{gathered}[b]
%     ℰ_{,uuu}(u₀, λ₀; v_i + λ_{,i}\dot{u}₀, v_j + λ_{,j} \dot{u}₀, V) + λ_{,j}ℰ_{,uuλ}(u₀, λ₀; v_i + λ_{,i} \dot{u}₀, V)\\
%     + ℰ_{,uu}(u₀, λ₀; w_{ij} + λ_{,ij} \dot{u}₀ + w_{,λ} + λ_{,i}λ_{,j} \ddot{u}₀, V)\\
%     + λ_{, ij} ℰ_{,uλ}(u₀, λ₀; V) + λ_{, i} ℰ_{,uuλ}(u₀, λ₀; v_j + λ_{,j} \dot{u}₀, V) + λ_{,i} λ_{,j} ℰ_{,uλλ}(u₀, λ₀; V) = 0
%   \end{gathered}
% \end{equation}
\begin{equation}
  \label{eq:20220901152145}
  \begin{gathered}[b]
    ℰ_{,uuu}(u₀, λ₀; v_i , v_j, V) + ℰ_{,uu}(u₀, λ₀; w_{ij}, V)\\
    +λ_{,i} \bigl[ℰ_{,uuu}(u₀, λ₀; v_j , \dot{u}₀, V) + ℰ_{,uuλ}[u₀, λ₀; v_j, V]\bigr]\\
    +λ_{,j} \bigl[ℰ_{,uuu}(u₀, λ₀; v_i , \dot{u}₀, V) + ℰ_{,uuλ}(u₀, λ₀; v_i, V)\bigr]\\
    +λ_{,ij} \bigl[ℰ_{,uu}(u₀, λ₀;  \dot{u}₀, V) + ℰ_{,uλ}(u₀, λ₀; V)\bigr]\\
    +λ_{,i} λ_{,j}\bigl[ℰ_{,uuu}(u₀, λ₀; \dot{u}₀ , \dot{u}₀, V) + 2ℰ_{,uuλ}(u₀, λ₀; \dot{u}₀, V) + ℰ_{,uλλ}(u₀, λ₀; V) + ℰ_{,uu}(u₀, λ₀; \ddot{u}₀, V)\bigr] = 0
  \end{gathered}
\end{equation}

The first term in Eq.~\eqref{eq:20220901152056} vanishes, since \(v_i ∈ V\);
besides, the last term also vanishes owing to Eq.~\eqref{eq:20220901143843};
therefore Eq.~\eqref{eq:20220901152056} is non-informative (identically
satisfied). Similarly, the terms in \(λ_{,ij}\) and \(λ_{,i} λ_{,j}\) in
Eq.~\eqref{eq:20220901152145} vanish owing to Eqs.~\eqref{eq:20220901143843} and
\eqref{eq:20220901143902}. Recognizing that the terms in \(λ_{,i}\) and
\(λ_{,j}\) are in fact derivatives of \(ℰ₂\) \wrt{} \(λ\) and that
\(ℰ₂(λ₀; w_{ij}, V)=0\) since \(V = \ker ℰ₂\), we get
\begin{equation}
    ℰ₃(λ₀; v_i , v_j, V) + λ_{,i} \dot{ℰ}₂(λ₀; v_j, V) + λ_{,j} \dot{ℰ}₂(λ₀; v_i, V) = 0.
\end{equation}

Since \(v_k ∈ V\), the above equation reads
\begin{equation}
...
The equation satisfied by \(λ_{,ij}\) is obtained by further differentiation of
Eq.~\eqref{eq:20220901125230} \wrt{} \(ξ^k\)
\begin{equation*}
  \begin{gathered}
    ℰ_{,uuuu}[v_i + w_{,i} + λ_{,i} (\dot{u}^\ast + w_{,λ}), v_j + w_{,j} + λ_{,j} (\dot{u}^\ast + w_{,λ}), v_k + w_{,k} + λ_{,k} (\dot{u}^\ast + w_{,λ}), \hat{v}]\\
    + λ_{,k} ℰ_{,uuuλ}[v_i + w_{,i} + λ_{,i} (\dot{u}^\ast + w_{,λ}), v_j + w_{,j} + λ_{,j} (\dot{u}^\ast + w_{,λ}), \hat{v}]\\
    + ℰ_{,uuu}[w_{,ik} + λ_{,i} w_{,kλ} + λ_{,k} w_{,iλ} + λ_{,ik} (\dot{u}^\ast + w_{,λ}) + λ_{,i} λ_{,k}(\ddot{u}^\ast + w_{,λλ}), v_j + w_{,j} + λ_{,j} (\dot{u}^\ast + w_{,λ}), \hat{v}]\\
    + ℰ_{,uuu}[v_i + w_{,i} + λ_{,i} (\dot{u}^\ast + w_{,λ}), w_{,jk} + λ_{,j} w_{,kλ} + λ_{,k} w_{,jλ} + λ_{,jk} (\dot{u}^\ast + w_{,λ}) + λ_{,j} λ_{,k}(\ddot{u}^\ast + w_{,λλ}), \hat{v}]\\
    + λ_{,jk}ℰ_{,uuλ}[v_i + w_{,i} + λ_{,i} (\dot{u}^\ast + w_{,λ}), \hat{v}]\\
    + λ_{,j}ℰ_{,uuuλ}[v_i + w_{,i} + λ_{,i} (\dot{u}^\ast + w_{,λ}), v_k + w_{,k} + λ_{,k} (\dot{u}^\ast + w_{,λ}), \hat{v}]\\
    + λ_{,j} λ_{,k} ℰ_{,uuλλ}[v_i + w_{,i} + λ_{,i} (\dot{u}^\ast + w_{,λ}), \hat{v}]\\
    + λ_{,j} ℰ_{,uuλ}[w_{,ik} + λ_{,i} w_{kλ} + λ_{,k} w_{iλ} + λ_{,ik} (\dot{u}^\ast + w_{,λ}) + λ_{,i} λ_{,k} (\ddot{u}^\ast + w_{,λλ}), \hat{v}]\\
    + ℰ_{,uuu}[w_{,ij} + λ_{,ij} (\dot{u}^\ast + w_{,λ}) + λ_{,i}λ_{,j} (\ddot{u}^\ast + w_{,λλ}), v_k + w_{,k} + λ_{,k} (\dot{u}^\ast + w_{,λ}), \hat{v}]\\
    + λ_{,k} ℰ_{,uuλ}[w_{,ij} + λ_{,ij} (\dot{u}^\ast + w_{,λ}) + λ_{,i}λ_{,j} (\ddot{u}^\ast + w_{,λλ}), \hat{v}]\\
    + ℰ_{,uu}[w_{,ijk} + λ_{,k} w_{,ij} + λ_{,ijk} (\dot{u}^\ast + w_{,λ}) + λ_{,ij} w_{,kλ} + \bigl(λ_{,ij} λ_{, k} + λ_{,ik}λ_{,j} + λ_{,jk} λ_{,i}\bigr) (\ddot{u}^\ast + w_{,λλ})\\
    + λ_{,i}λ_{,j} λ_{,k}(\dddot{u}^\ast + w_{,λλλ}) + λ_{,i} λ_{,j} w_{,kλλ}, \hat{v}]\\
    + λ_{, ijk} ℰ_{,uλ}(\hat{v}) + λ_{, ij} ℰ_{,uuλ}[v_k + w_{,k} + λ_{,k} (\dot{u}^\ast + w_{,λ}), \hat{v}] +  λ_{,k} λ_{, ij} ℰ_{,uλλ}(\hat{v})\\
    + λ_{, ik} ℰ_{,uuλ}[v_j + w_{,j} + λ_{,j} (\dot{u}^\ast + w_{,λ}), \hat{v}]\\
    + λ_{, i} ℰ_{,uuuλ}[v_j + w_{,j} + λ_{,j} (\dot{u}^\ast + w_{,λ}), v_k + w_{,k} + λ_{,k} (\dot{u}^\ast + w_{,λ}), \hat{v}]\\
    + λ_{, i} λ_{,k} ℰ_{,uuλλ}[v_j + w_{,j} + λ_{,j} (\dot{u}^\ast + w_{,λ}), \hat{v}]\\
    + λ_{, i} ℰ_{,uuλ}[w_{,jk} + λ_{,k} w_{,jλ}  + λ_{,j} w_{,kλ} + λ_{,jk} (\dot{u}^\ast + w_{,λ}) + λ_{,j} λ_{,k} (\ddot{u}^\ast + w_{,λλ}), \hat{v}]\\
    + \bigl(λ_{,ik} λ_{,j} + λ_{,i} λ_{,jk}\bigr) ℰ_{,uλλ}(\hat{v})\\
    + λ_{,i} λ_{,j} ℰ_{,uuλλ}(v_k + w_{,k} + λ_{,k} (\dot{u}^\ast + w_{,λ}), \hat{v})\\
    + λ_{,i} λ_{,j} λ_{,k} ℰ_{,uλλ}(\hat{v})= 0
  \end{gathered}
\end{equation*}
\begin{equation*}
  \begin{gathered}
    ℰ_{,uuuu}[v_i + λ_{,i} \dot{u}₀, v_j + λ_{,j} \dot{u}₀, v_k + λ_{,k} \dot{u}₀, \hat{v}]\\
    + λ_{,k} ℰ_{,uuuλ}[v_i + λ_{,i} \dot{u}₀, v_j + λ_{,j} \dot{u}₀, \hat{v}]\\
    + ℰ_{,uuu}[w_{ik} + λ_{,i} w_{kλ} + λ_{,k} w_i + λ_{,ik} \dot{u}₀ + λ_{,i} λ_{,k}\ddot{u}₀, v_j + λ_{,j} \dot{u}₀, \hat{v}]\\
    + ℰ_{,uuu}[v_i + λ_{,i} \dot{u}₀, w_{jk} + λ_{,j} w_k + λ_{,k} w_j + λ_{,jk} \dot{u}₀ + λ_{,j} λ_{,k}\ddot{u}₀, \hat{v}]\\
    + λ_{,jk}ℰ_{,uuλ}[v_i + λ_{,i} \dot{u}₀, \hat{v}]\\
    + λ_{,j}ℰ_{,uuuλ}[v_i + λ_{,i} \dot{u}₀, v_k + λ_{,k} \dot{u}₀, \hat{v}]\\
    + λ_{,j} λ_{,k} ℰ_{,uuλλ}[v_i + λ_{,i} \dot{u}₀, \hat{v}]\\
    + λ_{,j} ℰ_{,uuλ}[w_{ik} + λ_{,i} w_k + λ_{,k} w_i + λ_{,ik} \dot{u}₀ + λ_{,i} λ_{,k} \ddot{u}₀, \hat{v}]\\
    + ℰ_{,uuu}[w_{ij} + λ_{,ij} \dot{u}^\ast + λ_{,i}λ_{,j} \ddot{u}^\ast, v_k + λ_{,k} \dot{u}₀, \hat{v}]\\
    + λ_{,k} ℰ_{,uuλ}[w_{ij} + λ_{,ij} \dot{u}₀ + λ_{,i}λ_{,j} \ddot{u}₀, \hat{v}]\\
    + ℰ_{,uu}[w_{,ijk} + λ_{,k} w_{ij} + λ_{,ijk} \dot{u}₀ + λ_{,ij} w_k + \bigl(λ_{,ij} λ_{, k} + λ_{,ik}λ_{,j} + λ_{,jk} λ_{,i}\bigr) \ddot{u}₀ + λ_{,i}λ_{,j} λ_{,k} \dddot{u}₀ + λ_{,i} λ_{,j} w_{,kλλ}, \hat{v}]\\
    + λ_{, ijk} ℰ_{,uλ}(\hat{v}) + λ_{, ij} ℰ_{,uuλ}[v_k + λ_{,k} \dot{u}₀, \hat{v}] +  λ_{,k} λ_{, ij} ℰ_{,uλλ}(\hat{v})\\
    + λ_{, ik} ℰ_{,uuλ}[v_j + λ_{,j} \dot{u}₀, \hat{v}]\\
    + λ_{, i} ℰ_{,uuuλ}[v_j + λ_{,j} \dot{u}₀, v_k + λ_{,k} \dot{u}₀, \hat{v}]\\
    + λ_{, i} λ_{,k} ℰ_{,uuλλ}[v_j + λ_{,j} \dot{u}₀, \hat{v}]\\
    + λ_{, i} ℰ_{,uuλ}[w_{jk} + λ_{,k} w_j  + λ_{,j} w_k + λ_{,jk} \dot{u}₀ + λ_{,j} λ_{,k} \ddot{u}₀, \hat{v}]\\
    + \bigl(λ_{,ik} λ_{,j} + λ_{,i} λ_{,jk}\bigr) ℰ_{,uλλ}(\hat{v})\\
    + λ_{,i} λ_{,j} ℰ_{,uuλλ}(v_k  + λ_{,k} \dot{u}₀, \hat{v})\\
    + λ_{,i} λ_{,j} λ_{,k} ℰ_{,uλλ}(\hat{v})= 0
  \end{gathered}
\end{equation*}
...
\begin{equation*}
  \begin{gathered}
    ℰ_{,uuuu}[v_i, v_j, v_k, \hat{v}]\\
    + ℰ_{,uuu}[v_i, w_{jk}, \hat{v}]\\
    + ℰ_{,uuu}[v_j, w_{ik}, \hat{v}]\\
    + ℰ_{,uuu}[v_k, w_{ij}, \hat{v}]\\

    + λ_{,i} \bigl[
    ℰ_{,uuuu}(v_j, v_k, \dot{u}₀, \hat{v})
    + ℰ_{,uuuλ}(v_j, v_k, \hat{v})
    + ℰ_{,uuu}(w_k, v_j, \hat{v})
    + ℰ_{,uuu}(w_{jk}, \dot{u}₀, \hat{v})
    + ℰ_{,uuλ}(w_{jk}, \hat{v})
    \bigr]

    + λ_{,j} \bigl[
    ℰ_{,uuuu}(v_i, v_k, \dot{u}₀, \hat{v})
    + ℰ_{,uuuλ}(v_i, v_k, \hat{v})
    + ℰ_{,uuu}(w_k, v_i, \hat{v})
    + ℰ_{,uuu}(w_{ik}, \dot{u}₀, \hat{v})
    + ℰ_{,uuλ}(w_{ik}, \hat{v})
    \bigr]

    + λ_{,k} \bigl[
    ℰ_{,uuuu}(v_i, v_j, \dot{u}₀, \hat{v})
    + ℰ_{,uuuλ}(v_i, v_j, \hat{v})
    + ℰ_{,uuu}(w_i, v_j, \hat{v})
    + ℰ_{,uuu}(v_i, w_j, \hat{v})
    + ℰ_{,uuu}(w_{ij}, \dot{u}₀, \hat{v})
    + ℰ_{,uuλ}(w_{ij}, \hat{v})
    \bigr]

    + λ_{,i} λ_{,j} ℰ_{,uuuu}[\dot{u}₀, \dot{u}₀, v_k, \hat{v}]\\
    + λ_{,i} λ_{,j} ℰ_{,uuu}[w_{kλ}, \dot{u}₀, \hat{v}]\\
    + λ_{,i} λ_{,j} ℰ_{,uuu}[\dot{u}₀, w_k, \hat{v}]\\
    + λ_{,i} λ_{,j} ℰ_{,uuλ}[w_k, \hat{v}]\\
    + λ_{,i} λ_{,j} ℰ_{,uuuλ}[\dot{u}₀, v_k, \hat{v}]\\
    + λ_{,i} λ_{,j} ℰ_{,uuλ}[w_k, \hat{v}]\\
    + λ_{,i} λ_{,j} ℰ_{,uuu}[\ddot{u}₀, v_k, \hat{v}]\\
    + λ_{,i} λ_{,j} ℰ_{,uuuλ}[\dot{u}₀, v_k, \hat{v}]\\
    + λ_{,i} λ_{,j} ℰ_{,uuλλ}(v_k, \hat{v}) \\

    + λ_{,k} λ_{,j} ℰ_{,uuuλ}[v_i, \dot{u}₀, \hat{v}]\\
    + λ_{,j} λ_{,k} ℰ_{,uuu}[v_i, \ddot{u}₀, \hat{v}]\\
    + λ_{,j} λ_{,k} ℰ_{,uuλ}[ w_i, \hat{v}]\\
    + λ_{,j} λ_{,k} ℰ_{,uuuλ}[v_i, \dot{u}₀, \hat{v}]\\
    + λ_{,j} λ_{,k} ℰ_{,uuu}[w_i, \dot{u}₀, \hat{v}]\\
    + λ_{,j} λ_{,k} ℰ_{,uuλλ}(v_k, \hat{v}) \\
    + λ_{,j} λ_{,k} ℰ_{,uuuu}[v_i, \dot{u}₀, \dot{u}₀, \hat{v}]\\

    + λ_{,i} λ_{,k} ℰ_{,uuu}[\dot{u}₀, w_j, \hat{v}]\\
    + λ_{,i} λ_{,k} ℰ_{,uuuλ}[v_j,  \dot{u}₀, \hat{v}]\\
    + λ_{,i} λ_{,k} ℰ_{,uuλ}[w_j, \hat{v}]\\
    + λ_{,i} λ_{,k} ℰ_{,uuλλ}(v_k, \hat{v}) \\
    + λ_{,i} λ_{,k} ℰ_{,uuuλ}[\dot{u}₀, v_j, \hat{v}]\\
    + λ_{,i} λ_{,k} ℰ_{,uuuu}[\dot{u}₀, v_j, \dot{u}₀, \hat{v}]\\
    + λ_{,i} λ_{,k} ℰ_{,uuu}[\ddot{u}₀, v_j, \hat{v}]\\

    + λ_{,ij} ℰ_{,uuu}[v_k, \dot{u}₀, \hat{v}]\\
    + λ_{,ij} ℰ_{,uuλ}[v_k, \dot{u}₀, \hat{v}]\\

    + λ_{,jk} ℰ_{,uuu}[v_i, \dot{u}₀, \hat{v}]\\
    + λ_{,jk} ℰ_{,uuλ}[v_i, \hat{v}]\\

    + λ_{,ik} ℰ_{,uuu}[v_j, \dot{u}₀, \hat{v}]\\
    + λ_{,ik} ℰ_{,uuλ}[v_j, \hat{v}]\\

    + λ_{,i} λ_{,jk} \bigl[ ℰ_{,uuu}(\dot{u}₀, \dot{u}₀, \hat{v}) + 2ℰ_{,uuλ}(\dot{u}₀, \hat{v}) + ℰ_{,uλλ}(\hat{v}) \bigr]\\
    + λ_{,j} λ_{,ik} \bigl[ ℰ_{,uuu}(\dot{u}₀, \dot{u}₀, \hat{v}) + 2ℰ_{,uuλ}(\dot{u}₀, \hat{v}) +  ℰ_{,uλλ}(\hat{v})\bigr]\\
    + λ_{,k} λ_{,ij} \bigl[ ℰ_{,uuu}[\dot{u}₀, \dot{u}₀, \hat{v}] + 2ℰ_{,uuλ}(\dot{u}₀, \hat{v}) + ℰ_{,uλλ}(\hat{v}) \bigr]\\

    + λ_{, ijk} ℰ_{,uλ}(\hat{v})\\

    + λ_{,i} λ_{,j} λ_{,k} \bigl[ℰ_{,uuuu}[\dot{u}₀, \dot{u}₀, \dot{u}₀, \hat{v}] + 3ℰ_{,uuu}(\ddot{u}₀,  \dot{u}₀, \hat{v}) + 3ℰ_{,uuuλ}(\dot{u}₀, \dot{u}₀, \hat{v}) + 3ℰ_{,uuλ}(\ddot{u}₀, \hat{v}) + 3ℰ_{,uuλλ}(\dot{u}₀, \hat{v}) + ℰ_{,uλλ}(\hat{v})\bigr] = 0
  \end{gathered}
\end{equation*}
