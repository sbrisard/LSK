...
\begin{align}
  ℋ(λ)&=ℰ_{,uu}[u^\ast(λ), λ],\\
  \mathcal T(λ)&=ℰ_{,uuu}[u^\ast(λ), λ],\\
  \mathcal Q(λ)&=ℰ_{,uuuu}[u^\ast(λ), λ].
\end{align}
...
\(\mathcal T(λ; u, v, w)\), etc\dots. La dérivée de ces formes par
...
définie positive pour tout \(λ<λ_0\). Pour \(λ=λ_0\),
la forme quadratique \(ℋ(λ_0)\) n'est plus que positive. En
notant \(u₀=u^\ast(λ_0)\) la position d'équilibre obtenue pour la valeur
critique \(λ_0\) du paramètre de chargement \(λ\), on s'intéresse à
toutes les courbes d'équilibre qui passent par le point \((u₀, λ_0)\).

Noter que dans ce qui suit, on convient que les formes \(ℋ\),
\(\mathcal T\) et \(\mathcal Q\) sont implicitement évaluées en \(λ_0\)
lorsque \(λ\) n'est pas rappelé : ainsi, on notera
\(ℋ(•, •)\) plutôt que
\(ℋ(λ_0, •, •)\).

On écrit toute courbe d'équilibre passant par le point \((u₀, λ_0)\)
sous la forme paramétrique suivante
\begin{align}
  \label{eq:20211115075817}
  λ&=λ_0+ηλ₁+η²λ₂+η³λ₃+\cdots,\\
...
\begin{gather}
  ...
  \begin{aligned}[b]
    ...
    +3λ₁\bigl[\dot{\mathcal T}(u₁, u₁, \hat{u})
...
\begin{equation}
  \mathcal T_{ijk}ξ₁^jξ₁^k+2λ₁\dot{ℋ}_{ij}ξ₁^j=0.
\end{equation}
...
l'équation~\eqref{eq:20211112183220} s'écrit
\begin{equation}
  \mathcal T(u₁, u₁, \hat{u})+2λ₁\dot{ℋ}(u₁, \hat{u})
  +2ℋ(w₂, \hat{u})=0,
\end{equation}
...
\begin{equation}
  ...
  \begin{aligned}[b]
    ...
    +2λ₁\dot{\mathcal T}^\ast(u₁, \hat{u}, \hat{v})\\
...
On pose dans ce qui suit \(λ(η)=λ(η)-λ_0\) et
...
\begin{equation}
  ...
  f'(η)=\partial_η F[η, λ_0+λ(η)]
  +λ'(η)\,\partial_λ F[η, λ_0+λ(η)],
\end{equation}
\begin{equation}
  \begin{aligned}[b]
    \label{eq:20211112165810}
    f''(η)={}&\partial_{ηη}²F[η, λ_0+λ(η)]
    +2λ'(η)\,\partial_{ηλ}²F[η, λ_0+λ(η)]\\
    &+λ''(η)\,\partial_λ F[η, λ_0+λ(η)]
    +λ'(η)²\partial_{λλ}²F[η, λ_0+λ(η)]
  \end{aligned}
\end{equation}
et
\begin{equation}
  ...
  \begin{aligned}[b]
    f'''(η)={}&\partial_{ηηη}³F[η, λ_0+λ(η)]\\
    &+3λ'(η)\,\partial_{ηηλ}³F[η, λ_0+λ(η)]\\
    &+3λ'(η)²\partial_{ηλλ}³F[η, λ_0+λ(η)]\\
    &+3λ''(η)\,\partial_{ηλ}²F[η, λ_0+λ(η)]\\
    &+λ'''(η)\,\partial_λ F[η, λ_0+λ(η)]\\
    &+3λ'(η)\,λ''(η)\,\partial_{λλ}²F[η, λ_0+λ(η)]\\
    &+λ'(η)³\partial_{λλλ}³F[η, λ_0+λ(η)].
  \end{aligned}
\end{equation}
...
\begin{equation}
  ...
  \partial_{ηη}²F(0, λ)=\mathcal T(u₁, u₁, \hat{u})
  +2ℋ(u₂, \hat{u}).
\end{equation}
...
\begin{equation}
  ...
  \partial_{ηηλ}³F(0, λ)
  =\dot{\mathcal T}(u₁, u₁, \hat{u})+2\dot{ℋ}(u₂, \hat{u})
  \quad\text{et}\quad
  \partial_{ηλλ}³F(0, λ)
  =\ddot{ℋ}(u₁, \hat{u}).
\end{equation}
...
\begin{equation}
  ...
  \partial_η H(0, λ)=\mathcal T(u₁, \hat{u}, \hat{v}).
\end{equation}
...
\begin{equation}
  ...
  \partial_{ηη}²H(0, λ)=\mathcal Q(u₁, u₁, \hat{u}, \hat{v})
...
\begin{equation}
  ...
  \partial_{ηλ}²H(0, λ)=\dot{\mathcal T}(u₁, \hat{u}, \hat{v})
  \quad\text{et}\quad
...
\begin{equation}
  \bigl(\mathcal T_{ijk}^\astξₖ+λ₁\dot{ℋ}_{ij}^\ast\bigr)\chi ⱼ
  =\alpha₁\chi ᵢ,
\end{equation}
qui est un problème aux valeurs propres pour la matrice symétrique
\((\mathcal T_{ijk}^\astξₖ+λ₁\dot{ℋ}_{ij}^\ast)_{1≤i,
  j≤m}\)
