...
into Eq.~\eqref{eq:res3} and taking further \(\hat{u} = v_i\) [remember that
\(ℰ₂(λ₀; v_i, •) = 0\)], we then get
% \begin{multline*}
...
It is shown in Appendix~\ref{sec:20220616074108} that \((α₀, x_0)\) is
necessarily an eigenpair of \(ℰ₂(λ₀)\). Since \(ℰ₂ (λ₀)\) is positive,
\(α₀ ≥ 0\). If \(α₀>0\), then \(α>0\) in the neighborhood of \(λ₀\). Potentially
unstable modes are therefore such that \(α₀=0\). In other words, \(x₀ ∈ V\)
\begin{equation}
  \label{eq:20220609133608}
  x₀ &= χ₀^i v_i
\end{equation}
and it is further shown in Appendix~\ref{sec:20220616074108} that \((α₁, χ₀^i)\)
is an eigenpair of the symmetric tensor \((E_{ijk} ξ₁^k + λ₁ F_{ij})\)
\begin{equation}
  \label{eq:20220616074501}
  \bigl(E_{ijk} ξ₁^k + λ₁ F_{ij} \bigr) χ₀^j = α₁ χ₀^i.
\end{equation}

As for the higher order terms, it is found in Appendix~\ref{sec:20220616074108}
that
\begin{equation}
...
\section{Asymetric bifurcations}
...
In other words, \((α₀, x₀)\) is an eigenpair of \(ℰ₂(λ₀)\).
\pdfmarkupcomment{As shown in Sec.}{Reference}, only the case \(α₀\) is
relevant. Then \(x₀∈V\), which is expressed by the
expansion~\eqref{eq:20220609133608}.
...
\end{equation}
which reduces to Eq.~\eqref{eq:20220616074501}

On prend maintenant \(\hat{u} = \hat{w}∈W\) dans le problème
...
The solution to this problem is expressed as a linear combination of the \(w_i\)
and \(w_{ij}\) defined by the variational problems \eqref{eq:20220524134525} and
\eqref{eq:20220519164523}, respectively:
...
And Eq.~\eqref{eq:20220616082923} is retrieved.
