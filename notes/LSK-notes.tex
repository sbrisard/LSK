...
\begin{gather}
  ...
  \E₂(λ₀; w_i, \hat{w}) + \dot{\E}₂(λ₀; v_i, \hat{w}) = 0,\\
  \label{eq:20220519164523}
  \E₂(λ₀; w_{i j}, \hat{w})+\E₃(λ₀; v_i, v_j, \hat{w}) = 0,
\end{gather}
for all $\hat{w} ∈ W$. Since $w_{i}$ and $w_{ij}$ belong to $W$, we have
$〈 w_{i}, v 〉 = 〈 w_{ij}, v 〉 = 0$ for all $v ∈ V$. Since $\E₂(λ₀; •,
•)$ is symmetric, it can be verified that $w_{ij}=w_{ji}$. We also introduce
the following tensors, defined in $V$
\begin{gather}
  E_{ijk} = \E₃(λ₀; v_i, v_j, v_k) + \E₂(λ₀; v_i, w_{jk}) + \E₂(λ₀; v_j, w_{ki}) + \E₂(λ₀; v_k, w_{ij}),\\
  E_{ijkl} = \E₄(λ₀ ; v_i, v_j, v_k, v_l) + \E₃(λ₀ ; v_i, v_j, w_{kl}) + \E₃(λ₀ ; v_i, v_k, w_{lj}) + \E₃(λ₀ ; v_i, v_l, w_{jk}),\\
  F_{ij} = \dot{\E}₂(λ₀; v_i, v_j) + \E₂(λ₀; v_i, w_j) + \E₂(λ₀; v_j, w_i),
\end{gather}
as well as the derivatives
\begin{gather}
  \label{eq:20220615063626}
  \mathring{E}_{ijk} = \dot{\E}₃(λ₀; v_i, v_j, v_k) + \dot{\E₂}(λ₀; v_i, w_{jk}) + \dot{\E}₂(λ₀; v_j, w_{ki}) + \dot{\E}₂(λ₀; v_k, w_{ij}),\\
  \label{eq:20220615063633}
  \mathring{F}_{ij} = \ddot{\E}₂(λ₀; v_i, v_j) + \dot{\E}₂(λ₀; v_i, w_j) + \dot{\E}₂(λ₀; v_j, w_i).
\end{gather}

Note that, since $\E₂(λ₀; v_i, •) = 0$, the above expressions simplify as follows
\begin{gather}
  \label{eq:20220524135619}
  E_{ijk} = \E₃(λ₀; v_i, v_j, v_k),\\
  \label{eq:20220524135553}
  E_{ijkl} = \E₄(λ₀ ; v_i, v_j, v_k, v_l) + \E₃(λ₀ ; v_i, v_j, w_{kl}) + \E₃(λ₀ ; v_i, v_k, w_{jl}) + \E₃(λ₀ ; v_i, v_l, w_{jk}),\\
  \label{eq:20220524135643}
  F_{ij} = \dot{\E}₂(λ₀; v_i, v_j).
\end{gather}
...
\begin{equation}
  \begin{aligned}[b]
    \E(u₀ + u, λ₀) - \E(u₀, λ₀) ={}
    &\tfrac{1}{2} \E₂(λ₀; u, u) + \tfrac{1}{6} \E₃(λ₀; u, u, u)\\
    &+ \tfrac{1}{24} \E₄(λ₀; u, u, u, u) + o(〈 u , u 〉²),
  \end{aligned}
\end{equation}
where the linear term has been omitted, $u₀$ being a critical point of the
energy. Since $v ∈ V$, we have $\E₂(λ₀; v, •) = 0$. We now expand $u$ as
...
\begin{equation}
  \begin{aligned}[b]
    \E(u₀ + u, λ₀) - \E(u₀, λ₀) ={}
    & \tfrac{1}{2} η² \E₂(λ₀; w, w) + \tfrac{1}{6} ξ³ \E₃(λ₀; v, v, v)\\
    & + \tfrac{1}{2} ξ² η \E₃(λ₀; v, v, w) + \tfrac{1}{2} ξ η² \E₃(λ₀; v, w, w)\\
    & + \tfrac{1}{6} η³ \E₃(λ₀; w, w, w) + \tfrac{1}{24} ξ⁴ \E₄(λ₀; v, v, v, v)\\
    & + \tfrac{1}{6} ξ³ η \E₄(λ₀; v, v, v, w) + \tfrac{1}{4} ξ² η² \E₄(λ₀; v, v, w, w)\\
    & + \tfrac{1}{6} ξ η³ \E₄(λ₀; v, w, w, w) + \tfrac{1}{24} η⁴ \E₄(λ₀; w, w, w, w)\\
...
\begin{equation}
  ...
  \E₃(λ₀; v, v, v) = 0 \quad \text{and} \quad \E₄(λ₀; v, v, v, v) \geq 0 \quad \text{for all} \quad v∈V.
\end{equation}
...
In other words, if there exists $v ∈ V$ such that $\E₃(λ₀; v, v, v) \neq 0$
...
\begin{equation}
  \begin{aligned}[b]
    \E(u₀ + u, λ₀) - \E(u₀, λ₀) ={} & \tfrac{1}{2} ξ⁴ \bigl[ \E₂(λ₀; w, w) + \E₃(λ₀; v, v, w)\\
    & + \tfrac{1}{12} \E₄(λ₀; v, v, v, v) \bigr] + o(ξ⁴)
  \end{aligned}
\end{equation}
...
Note that Eq.~\eqref{eq:20211109145224} implies $\E₄(λ₀; v, v, v, v) ≥ 0$,
...
\begin{equation}
  \E_{,u}[u^\ast(λ) + ξ_i v_i + w, λ; \hat{u}] = 0, \quad \text{for all} \quad \hat{u} ∈ U
\end{equation}
or, equivalently
\begin{equation}
  \label{eq:20220901120544}
  \E_{,u}[u^\ast(λ) + ξ_i v_i + w, λ; \hat{v}] = 0, \quad \text{for all} \quad \hat{v} ∈ V
\end{equation}
and
\begin{equation}
  \label{eq:20220825143616}
  \E_{,u}[u^\ast(λ) + ξ_i v_i + w, λ; \hat{w}] = 0, \quad \text{for all} \quad \hat{w} ∈ W.
\end{equation}
...
\begin{equation}
  ...
  \E_{,uu}(u^\ast + ξ_k v_k + w, λ; v_i + w_{,i}, \hat{w}) = 0.
\end{equation}

Substituting $ξ_1 = \cdots = ξ_m = 0, λ = λ₀$ in the above equations and
observing that $\E₂(λ₀; v_i, W) = 0$ since $v_i ∈ V$, we get
\begin{equation}
\label{eq:20220825150219}
  \E₂(λ₀; v_i + w_{,i}, \hat{w}) = \E₂(λ₀; w_{,i}, \hat{w}) = 0.
\end{equation}
...
% \begin{equation}
%   \begin{gathered}[b]
%     \E_{,uuu}(u₀, λ₀; v_i + λ_{,i}\dot{u}₀, v_j + λ_{,j} \dot{u}₀, \hat{v}) + λ_{,j}\E_{,uuλ}(u₀, λ₀; v_i + λ_{,i} \dot{u}₀, \hat{v})\\
%     + \E_{,uu}(u₀, λ₀; w_{ij} + λ_{,ij} \dot{u}₀ + w_{,λ} + λ_{,i}λ_{,j} \ddot{u}₀, \hat{v})\\
%     + λ_{, ij} \E_{,uλ}(u₀, λ₀; \hat{v}) + λ_{, i} \E_{,uuλ}(u₀, λ₀; v_j + λ_{,j} \dot{u}₀, \hat{v}) + λ_{,i} λ_{,j} \E_{,uλλ}(u₀, λ₀; \hat{v}) = 0
%   \end{gathered}
% \end{equation}
\begin{equation}
...
\begin{equation}
  \begin{aligned}[b]
    \E_{,uuu}(u₀, λ₀; v_i , v_j, \hat{v}) + λ_{,i} \bigl[ \underbrace{\E_{,uuu}(u₀, λ₀; v_j , \dot{u}₀, \hat{v}) + \E_{,uuλ}(u₀, λ₀; v_j, \hat{v})}_{=\dot{\E}₂(λ₀; v_j, \hat{v})} \bigr]&\\
    +λ_{,j} \bigl[ \underbrace{\E_{,uuu}(u₀, λ₀; v_i , \dot{u}₀, \hat{v}) + \E_{,uuλ}(u₀, λ₀; v_i, \hat{v})}_{λ_{,j} \dot{\E}₂(λ₀; v_i, \hat{v})} \bigr] &= 0
  \end{aligned}
\end{equation}
and, recognizing derivatives of $\E₂$ with respect to $λ$, we finally get
\begin{equation}
    \E₃(λ₀; v_i , v_j, \hat{v}) + λ_{,i} \dot{\E}₂(λ₀; v_j, \hat{v}) + λ_{,j} \dot{\E}₂(λ₀; v_i, \hat{v}) = 0.
\end{equation}
Testing with $v_k ∈ V$, the above equation reads
\begin{equation}
  \E₃(λ₀; v_i , v_j, v_k) + λ_{,i} \dot{\E}₂(λ₀; v_j, v_k) + λ_{,j} \dot{\E}₂(λ₀; v_i, v_k) = 0,
\end{equation}
...
\eqref{eq:20211115075835}. In other words, the residual $\E_{, u} [u(η), λ(η)]$
...
\begin{equation}
  ...
  \E₂(λ₀; \order[1]u, \hat{u}) = 0,
\end{equation}
\begin{equation}
  \label{eq:20220524133447}
  \E₃(λ₀; \order[1]u, \order[1]u, \hat{u}) + 2\order[1]λ\dot{\E}₂(λ₀; \order[1]u, \hat{u}) + \E₂(λ₀; \order[2]u, \hat{u}) = 0,
\end{equation}
\begin{equation}
  \label{eq:20220708060436}
  \begin{aligned}[b]
    \E₄(λ₀; \order[1]u, \order[1]u, \order[1]u, \hat{u}) + 3\E₃(λ₀; \order[1]u, \order[2]u, \hat{u}) + \E₂(λ₀; \order[3]u, \hat{u})&\\
    + 3\order[1]λ\dot{\E}₃(λ₀; \order[1]u, \order[1]u, \hat{u}) + 3\order[1]λ\dot{\E}₂(λ₀;  \order[2]u, \hat{u})&\\
    + 3(\order[1]λ)^2\ddot{\E}₂(λ₀; \order[1]u, \hat{u}) + 3\order[2]λ\dot{\E}₂(λ₀; \order[1]u, \hat{u}) & = 0.
  \end{aligned}
\end{equation}
...
\begin{equation}
  ...
  \tfrac{1}{2} \E₃(λ₀; \order[1]u, \order[1]u, \hat{v}) + \order[1]λ\dot{\E}₂(λ₀; \order[1]u, \hat{v}) = 0,
\end{equation}
...
\begin{equation}
 \E₃(λ₀; \order[1]u, \order[1]u, \hat{u}) + 2\order[1]λ \dot{\E}₂(λ₀; \order[1]u, \hat{u}) + \E₂(λ₀; \order[2]w, \hat{u}) = 0,
\end{equation}
...
\begin{equation}
  ...
  \E₂(λ₀; \order[2]w, \hat{w}) + \order[1]{ξ_i} \order[1]{ξ_j} \E₃(λ₀; v_i, v_j, \hat{w}) + 2\order[1]λ \order[1]{ξ_i} \dot{\E}₂(λ₀; v_i, \hat{w}) = 0,
\end{equation}
...
[remember that $\E₂(λ₀; v_i, •) = 0$], we then get
% \begin{equation*}
%   \begin{aligned}[b]
%     \E₄(λ₀; v_i, \order[1]{ξ_j} v_j, \order[1]{ξ_k} v_k, \order[1]{ξ_l} v_l)
%     + 3\E₃(λ₀; v_i, \order[1]{ξ_j} v_j, \order[2]{ξ_k} v_k + \order[1]{ξ_k} \order[1]{ξ_l} w_{kl}
%     + 2\order[1]λ \order[1]{ξ_k} w_k)&\\
%   + 3\order[1]λ \dot{\E}₃(λ₀; v_i, \order[1]{ξ_j} v_j, \order[1]{ξ_k} v_k)
%     + 3\order[1]λ \dot{\E}₂(λ₀; v_i, \order[2]{ξ_j} v_j + \order[1]{ξ_j} \order[1]{ξ_k} w_{jk} + 2\order[1]λ \order[1]{ξ_j} w_j)&\\
%     + 3( \order[1]λ )^2 \ddot{\E}₂(λ₀; v_i, \order[1]{ξ_j} v_j) + 3\order[2]λ \dot{\E}₂(λ₀; v_i, \order[1]{ξ_j} v_j) &= 0
%   \end{aligned}
% \end{equation*}
% \begin{equation*}
%   \begin{aligned}[b]
%     \E₄(λ₀; v_i, v_j, v_k, v_l) \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l}
%     + 3\E₃(λ₀; v_i, v_j, v_k) \order[1]{ξ_j} \order[2]{ξ_k}
%     + 3\E₃(λ₀; v_i, v_j, w_{kl}) \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l}&\\
%     + 6\order[1]λ \E₃(λ₀; v_i, v_j, w_k) \order[1]{ξ_j} \order[1]{ξ_k}
%     + 3\order[1]λ \dot{\E}₃(λ₀; v_i, v_j, v_k) \order[1]{ξ_j} \order[1]{ξ_k}
%     + 3\order[1]λ \dot{\E}₂(λ₀; v_i, v_j) \order[2]{ξ_j}&\\
%     + 3\order[1]λ \dot{\E}₂(λ₀; v_i, w_{jk}) \order[1]{ξ_j} \order[1]{ξ_k}
%     + 6( \order[1]λ )^2 \dot{\E}₂(λ₀; v_i, w_j) \order[1]{ξ_j}
%     + 3( \order[1]λ )^2 \ddot{\E}₂(λ₀; v_i, v_j) \order[1]{ξ_j}&\\
%     + 3\order[2]λ \dot{\E}₂(λ₀; v_i, v_j) \order[1]{ξ_j} &= 0
%   \end{aligned}
% \end{equation*}
\begin{equation*}
  \begin{aligned}[b]
    \bigl[\E₄(λ₀; v_i, v_j, v_k, v_l) + 3\E₃(λ₀; v_i, v_j, w_{kl})\bigr] \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l}&\\
    + 3\order[1]λ \bigl[2\E₃(λ₀; v_i, v_j, w_k) + \dot{\E}₃(λ₀; v_i, v_j, v_k) + \dot{\E}₂(λ₀; v_i, w_{jk}) \bigr] \order[1]{ξ_j} \order[1]{ξ_k}&\\
    + 3 \bigl\{ ( \order[1]λ )^2 \bigl[ 2 \dot{\E}₂(λ₀; v_i, w_j) + \ddot{\E}₂(λ₀; v_i, v_j) \bigr] + \order[2]λ \dot{\E}₂(λ₀; v_i, v_j) \bigr\}\order[1]{ξ_j}&\\
    + 3\bigl[\E₃(λ₀; v_i, v_j, v_k) \order[1]{ξ_k} + \order[1]λ \dot{\E}₂(λ₀; v_i, v_j)\bigr] \order[2]{ξ_j} &= 0
  \end{aligned}
\end{equation*}
It results from the variational problems \eqref{eq:20220524134525} and
\eqref{eq:20220519164523} that
\begin{equation*}
  \E₃(λ₀; v_i, v_j, w_k) = -\E₂(λ₀ ; w_{ij}, w_k) = \dot{\E}₂(λ₀; v_k, w_{ij}),
\end{equation*}
therefore
\begin{equation*}
  \begin{aligned}[b]
    \E₃(λ₀; v_i, v_j, w_k) \order[1]{ξ_j} \order[1]{ξ_k} &= \tfrac{1}{2} \bigl[ \E₃(λ₀; v_i, v_j, w_k) + \E₃(λ₀; v_i, v_k, w_j)\bigr] \order[1]{ξ_j} \order[1]{ξ_k}\\
                                    &= \tfrac{1}{2} \bigl[ \dot{\E}₂(λ₀; v_k, w_{ij}) + \dot{\E}₂(λ₀; v_j, w_{ik}) \bigr] \order[1]{ξ_j} \order[1]{ξ_k}.
  \end{aligned}
\end{equation*}
Similarly,
\begin{equation*}
  \begin{aligned}[b]
    \dot{\E}₂(λ₀; v_i, w_j) &= -\E₂(λ₀; w_i, w_j) = -\E₂(λ₀; w_j, w_i) = \dot{\E}₂(λ₀; v_j, w_i)\\
                           &= \tfrac{1}{2} \bigl[ \dot{\E}₂(λ₀; v_i, w_j) + \dot{\E}₂(λ₀; v_j, w_i) \bigr].
  \end{aligned}
\end{equation*}
% \begin{equation*}
%   \begin{aligned}[b]
%     \bigl[\E₄(λ₀; v_i, v_j, v_k, v_l) + \E₃(λ₀; v_i, v_j, w_{kl}) + \E₃(λ₀; v_i, v_k, w_{jl}) + \E₃(λ₀; v_i, v_l, w_{jk}) \bigr] \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l}&\\
%   + 3\order[1]λ \bigl[\dot{\E}₃(λ₀; v_i, v_j, v_k) + \dot{\E}₂(λ₀; v_i, w_{jk}) + \dot{\E}₂(λ₀; v_j, w_{ik}) + \dot{\E}₂(λ₀; v_k, w_{ij}) \bigr] \order[1]{ξ_j} \order[1]{ξ_k}&\\
%   + 3( \order[1]λ )^2 \bigl[ \ddot{\E}₂(λ₀; v_i, v_j) + \dot{\E}₂(λ₀; v_i, w_j) + \dot{\E}₂(λ₀; v_j, w_i) \bigr] \order[1]{ξ_j}&\\
%   + 3\bigl[\E₃(λ₀; v_i, v_j, v_k) \order[1]{ξ_k} + \order[1]λ \dot{\E}₂(λ₀; v_i, v_j)\bigr] \order[2]{ξ_j} + 3\order[2]λ \dot{\E}₂(λ₀; v_i, v_j) \order[1]{ξ_j} &= 0
%   \end{aligned}
% \end{equation*}
...
fundamental branch
\begin{equation}
 \E₂(λ; \hat{u}, \hat{v}) = \E₂(λ₀; \hat{u}, \hat{v}) + \bigl(λ - λ₀\bigr) \dot{\E}₂(λ₀; \hat{u}, \hat{v}) + o(λ - λ₀).
\end{equation}

In what follows, it will be assumed that $\dot{\E}₂(λ₀)≠0$ and that $\E₂(λ)$
...
results that $\dot{\E}₂(λ₀)$ is negative definite over $V$. In other words,
...
\begin{equation}
  ...
  \begin{aligned}[b]
    \E_{, uu}[u(η), λ(η); \hat{u}, \hat{v}] ={}
    & \E₂(λ₀ ; \hat{u}, \hat{v}) + η \bigl[\E₃(λ₀ ; \order[1]u, \hat{u}, \hat{v})  + \order[1]λ \dot{\E}₂(λ₀; \hat{u}, \hat{v})\bigr]\\
    &+ \tfrac{1}{2} η² \bigl[\E₄(λ₀; \order[1]u, \order[1]u, \hat{u}, \hat{v}) + \E₃(λ₀; \order[2]u, \hat{u}, \hat{v})\\
    & + 2\order[1]λ \dot{\E}₃(λ₀; \order[1]u, \hat{u}, \hat{v}) + ( \order[1]λ )² \ddot{\E}₂(λ₀; \hat{u}, \hat{v})\\
    & + \order[2]λ \dot{\E}₂(λ₀; \hat{u}, \hat{v}) \bigr] + o(η²).
  \end{aligned}
\end{equation}
...
\begin{equation}
  ...
  \E_{, u u} [u(η), λ(η); x, \hat{u}] = α 〈 x, \hat{u} 〉 \quad \text{for all} \quad \hat{u} ∈ V,
\end{equation}
...
$(\order[0]α, x_0)$ is necessarily an eigenpair of $\E₂(λ₀)$. Since $\E₂ (λ₀)$ is
...
\begin{equation}
  ...
  \begin{aligned}[b]
    \E[u(η), λ(η)] ={} & \E\{u^{\ast}[λ(η)], λ(η)\} + \tfrac{1}{6} \order[1]λ η³ F_{i j} \order[1]{ξ_i} \order[1]{ξ_j} + \tfrac{1}{24} η⁴ \bigl\{E_{ijkl} \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l}\\
...
\begin{equation}
  \E[u(η), λ(η)] = \E\{u^{\ast}[λ(η)], λ(η)\} + \tfrac{1}{6} \order[1]λ η³ F_{i j} \order[1]{ξ_i} \order[1]{ξ_j} + o(η³).
\end{equation}
...
\begin{equation}
  \begin{aligned}[b]
    \E[u(λ), λ] &= \E[u^{\ast}(λ), λ] + \frac{\bigl(λ - λ₀\bigr)³}{6\bigl( \order[1]λ \bigr)^2} F_{i j} \order[1]{ξ_i} \order[1]{ξ_j} + o(λ³)\\
    &= \E[u^{\ast}(λ), λ] + \frac{2 \bigl( F_{i j} \order[1]{ξ_i} \order[1]{ξ_j} \bigr)³}{3 \bigl( E_{ijk} \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} \bigr)²} \bigl(λ - λ₀\bigr)³ + o(λ³).
  \end{aligned}
\end{equation}
...
\begin{equation}
  ...
  \begin{aligned}[b]
    \E[u(η), λ(η)]
    &= \E\{u^{\ast}[λ(η)], λ(η)\} + \tfrac{1}{24} η⁴ \bigl(E_{ijkl} \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l}  + 6  \order[2]λ F_{ij} \order[1]{ξ_i} \order[1]{ξ_j}\bigr) + o(η⁴)\\
    &= \E\{u^{\ast}[λ(η)], λ(η)\} - \tfrac{1}{24} η⁴ E_{ijkl} \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l} + o(η⁴).
  \end{aligned}
\end{equation}
...
\begin{equation}
  \E[u(η), λ(η)] = \E\{u^{\ast}[λ(η)], λ(η)\} - \frac{3 \bigl( F_{ij} \order[1]{ξ_i} \order[1]{ξ_j} \bigr)²}{2 E_{ijkl} \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l} } \bigl( λ - λ₀ \bigr)²  + o(λ²).
\end{equation}
...
$\dot{\E}₂(λ₀)$ is negative definite over $V$, we have $F_{11} < 0$.
...
\begin{equation}
  \begin{aligned}[b]
    \E_{, uu}[u(η), λ(η), v_1, v_1] &= η \bigl(E_{111} \order[1]{ξ_1} + \order[1]λ F_{11}\bigr) + o(η) = -2 η F_{11} + o(η)\\
...
\begin{equation*}
  \E_{, uu}[u(η), λ(η); v_1, v_1] = \tfrac{1}{2} η² \bigl[ E_{1111}\bigl(ξ_1^1\bigr) ^2 + \order[2]λ F_{11} \bigr] + o(η²) = - η² \order[2]λ F_{11} + o(η²),
\end{equation*}
...
\begin{equation}
  ...
  f(η) = \E_{, u} [u(η), λ(η); \hat{u}]
  \quad \text{and} \quad
  F(η, λ) = \E_{, u}[u^{\ast}(λ) + U(η), λ; \hat{u}],
\end{equation}
the test function $\hat{u}$ being fixed. It is first observed that
$F(0, λ) = \E_{, u} [u^{\ast} (λ), λ; \hat{u}] = 0$, since $u^{\ast}(λ)$ is
...
\begin{equation*}
  ∂_{η}F(η, λ) = \E_{, u u}[u^{\ast}(λ) + U(η), λ; U'(η), \hat{u}],
\end{equation*}
\begin{equation*}
  \begin{aligned}[b]
    ∂_{η η}² F(η, λ) ={}
    & \E_{, uuu}[u^{\ast}(λ) + U(η), λ; U'(η), U'(η), \hat{u}]\\
    & + \E_{, uu} [u^{\ast}(λ) + U(η), λ; U''(η), \hat{u}],
  \end{aligned}
\end{equation*}
\begin{equation*}
  \begin{aligned}[b]
    ∂_{ηηη}³ F(η, λ) ={}
    & \E_{, uuuu}[u^{\ast}(λ) + U(η), λ; U'(η), U'(η), U'(η), \hat{u}]\\
    & + 3\E_{, u u u}[u^{\ast}(λ) + U(η), λ; U'(η), U''(η), \hat{u}]\\
    & + \E_{, uu}[u^{\ast}(λ) + U(η), λ; U'''(η), \hat{u}],
  \end{aligned}
\end{equation*}
and, at $η = 0$
\begin{gather*}
  ∂_{η}F(0, λ) = \E₂(λ; \order[1]u, \hat{u}),\\
  ∂_{ηη}² F(0, λ) = \E₃(λ; \order[1]u, \order[1]u, \hat{u}) + \E₂(λ; \order[2]u, \hat{u}),\\
  ∂_{ηηη}³ F(0, λ) = \E₄(λ; \order[1]u, \order[1]u, \order[1]u, \hat{u}) + 3\E₃(λ; \order[1]u, \order[2]u, \hat{u}) + \E₂(λ; \order[3]u, \hat{u}).
\end{gather*}
Upon derivation with respect to $λ$, we find successively
\begin{gather*}
  ∂_{ηλ}² F(0, λ) = \dot{\E}₂(λ; \order[1]u, \hat{u}),\\
  ∂_{ηλλ}³ F(0, λ) = \ddot{\E}₂(λ; \order[1]u, \hat{u}),\\
  ∂_{ηηλ}³ F(0, λ) = \dot{\E}₃(λ; \order[1]u, \order[1]u, \hat{u}) + \dot{\E₂}(λ; \order[2]u, \hat{u}).
\end{gather*}
...
$η=0$
\begin{gather*}
  f'(0) = \E₂(λ₀; \order[1]u, \hat{u}),\\
  f''(0) = \E₃(λ₀; \order[1]u, \order[1]u, \hat{u}) + \E₂(λ₀; \order[2]u, \hat{u}) + 2 \order[1]λ \dot{\E}₂(λ₀; \order[1]u, \hat{u}),\\
  \begin{aligned}[b]
    f'''(0) ={}
    & \E₄(λ₀; \order[1]u, \order[1]u, \order[1]u, \hat{u}) + 3\E₃(λ₀; \order[1]u, \order[2]u, \hat{u}) + \E₂(λ₀ ; \order[3]u, \hat{u})\\
    & + 3\order[1]λ \dot{\E}₃(λ₀; \order[1]u, \order[1]u, \hat{u}) + 3\order[1]λ \dot{\E}₂(λ₀; \order[2]u, \hat{u})\\
    & + 3 \bigl( \order[1]λ \bigr)^2 \ddot{\E}₂(λ₀; \order[1]u, \hat{u}) + 3 \order[2]λ \dot{\E}₂(λ₀; \order[1]u, \hat{u}),
  \end{aligned}
\end{gather*}
...
\begin{equation}
  ...
  \begin{gathered}[b]
    \E_{, u}[u(η), λ(η)] ={} η \E₂(λ₀; \order[1]u, \hat{u}) + \tfrac{1}{2} η² \bigl[\E₃(λ₀; \order[1]u, \order[1]u, \hat{u})  + \E₂(λ₀; \order[2]u, \hat{u})\\
    {} + 2 \order[1]λ \dot{\E}₂(λ₀; \order[1]u, \hat{u})\bigr] + \tfrac{1}{6} η³ \bigl[ \E₄(λ₀; \order[1]u, \order[1]u, \order[1]u, \hat{u}) + 3\E₃(λ₀; \order[1]u, \order[2]u, \hat{u})\\
    {} + \E₂(λ₀; \order[3]u, \hat{u}) + 3\order[1]λ \dot{\E}₃(λ₀; \order[1]u, \order[1]u, \hat{u}) + 3\order[1]λ \dot{\E}₂(λ₀; \order[2]u, \hat{u})\\
    {} + 3 \bigl( \order[1]λ \bigr)^2 \ddot{\E}₂(λ₀; \order[1]u, \hat{u}) + 3 \order[2]λ \dot{\E}₂(λ₀ ; \order[1]u, \hat{u}) \bigr] + o(η³),
  \end{gathered}
\end{equation}
...
\begin{equation}
  F(η, λ) = \E[u^{\ast}(λ) + U(η), λ] - \E[u^{\ast}(λ), λ]
  \quad \text{et} \quad
  f(η) = F [η, λ₀ + Λ(η)].
\end{equation}
...
\begin{gather*}
  ∂_{η} F(η, λ) = \E_{, u}(U'),\\
  ∂_{ηη}² F(η, λ) = \E_{, uu} (U', U') + \E_{, u} (U''),\\
  ∂_{ηηη}³ F(η, λ) = \E_{, uuu}(U', U', U') + 3\E_{, uu}(U', U'') + \E_{, u}(U'''),\\
  \begin{aligned}[b]
    ∂_{ηηηη}⁴ F ={}
...
$η$. For $η = 0$, observing that $\E_{, u}[u^{\ast}(λ), λ] = 0$
\begin{gather*}
  ∂_{η} F(0, λ) = 0,\\
  ∂_{ηη}² F(0, λ) =\E₂(λ ; \order[1]u, \order[1]u),\\
  ∂_{ηηη}³ F(0, λ) = \E₃(λ; \order[1]u, \order[1]u, \order[1]u) + 3\E₂(λ; \order[1]u, \order[2]u),\\
  \begin{aligned}[b]
    ∂_{ηηηη}⁴ F(η, λ) ={} & \E₄(λ; \order[1]u, \order[1]u, \order[1]u, \order[1]u) + 6\E₃(λ; \order[1]u, \order[1]u, \order[2]u)\\
    & + 3\E₂(λ; \order[2]u, \order[2]u) + 4\E₂(λ; \order[1]u, \order[3]u),
  \end{aligned}
\end{gather*}
...
\begin{equation*}
  \begin{gathered}
    ∂_{ηλ}² F(0, λ) = 0,\\
    ∂_{ηηλ}³ F(0, λ) = \dot{\E}₂(λ; \order[1]u, \order[1]u),\\
    ∂_{ηλλ}³ F(0, λ) = 0,\\
  \end{gathered}
  \qquad
  \begin{gathered}
    ∂_{ηηηλ}⁴ F(0, λ) = \dot{\E}₃(λ; \order[1]u, \order[1]u, \order[1]u) + 3\dot{\E}₂(λ; \order[1]u, \order[2]u),\\
    ∂_{ηηλλ}⁴ F(0, λ) = \ddot{\E}₂(λ; \order[1]u, \order[1]u),\\
    ∂_{ηλλλ}⁴ F(0, λ) = 0
  \end{gathered}
\end{equation*}
and finally
\begin{gather}
  f'(0) = 0,\\
  f''(0) = \E₂(λ₀; \order[1]u, \order[1]u),\\
  f'''(0) =\E₃(λ₀; \order[1]u, \order[1]u, \order[1]u) + 3\E₂(λ₀; \order[1]u, \order[2]u) + 3\order[1]λ \dot{\E}₂(λ₀; \order[1]u, \order[1]u),\\
  \label{eq:20220905063614}
  \begin{aligned}[b]
    f''''(0) ={}
    & \E₄(λ₀; \order[1]u, \order[1]u, \order[1]u, \order[1]u) + 6\E₃(λ₀; \order[1]u, \order[1]u, \order[2]u) + 3\E₂(λ₀; \order[2]u, \order[2]u)\\
    & + 4\E₂(λ₀; \order[1]u, \order[3]u) + 4 \order[1]λ \dot{\E}₃(λ₀; \order[1]u, \order[1]u, \order[1]u) + 12 \order[1]λ \dot{\E}₂(λ₀; \order[1]u, \order[2]u)\\
    & + 6( \order[1]λ )^2 \ddot{\E}₂(λ₀; \order[1]u, \order[1]u) + 6\order[2]λ \dot{\E}₂(λ₀; \order[1]u, \order[1]u).
  \end{aligned}
\end{gather}

Since $\order[1]u∈V$, we have $\E₂(λ₀; \order[1]u, \order[k]u) = 0$ for
...
\begin{equation*}
  \begin{aligned}[b]
    ...
    ={} & \E₂(\order[2]{ξ_i} v_i + \order[1]{ξ_i} \order[1]{ξ_j} w_{i j} + 2\order[1]λ \order[1]{ξ_i} w_i, \order[2]{ξ_k} v_k + \order[1]{ξ_k} \order[1]{ξ_l} w_{k l} + 2\order[1]λ \order[1]{ξ_k} w_k)\\
    ={} & \E₂(\order[1]{ξ_i} \order[1]{ξ_j} w_{i j} + 2 \order[1]λ \order[1]{ξ_i} w_i, \order[1]{ξ_k} \order[1]{ξ_l} w_{k l} + 2 \order[1]λ \order[1]{ξ_k} w_k)\\
    ={} & \E₂(w_{i j}, w_{k l}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l} + 4 \order[1]λ \E₂(w_i, w_{j k}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k}\\
    & + 4 ( \order[1]λ )^2 \E₂(w_i, w_j) \order[1]{ξ_i} \order[1]{ξ_j}\\
    ={} & \E₂(w_{i j}, w_{k l}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l} + 4 \order[1]λ \E₂(w_i, w_{j k}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k}\\
    &+ 2 ( \order[1]λ )^2 \bigl[\E₂(w_i, w_j) + \E₂(w_j, w_i)\bigr] \order[1]{ξ_i} \order[1]{ξ_j}\\
    ={} & -\E₃(v_i, v_j, w_{k l}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l} - 4 \order[1]λ \dot{\E}₂ (v_i, w_{j k}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k}\\
    & - 2 ( \order[1]λ )^2 \bigl[\dot{\E}₂(v_i, w_j) + \dot{\E}₂(v_j, w_i)\bigr] \order[1]{ξ_i} \order[1]{ξ_j}
  \end{aligned}
\end{equation*}
finally
\begin{equation*}
  \begin{aligned}[b]
    ...
    ={} & \dot{\E}₂ (v_i, v_j) \order[1]{ξ_i} \order[2]{ξ_j} + \dot{\E}₂(v_i, w_{j k}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} + 2\order[1]λ \dot{\E}₂(v_i, w_j) \order[1]{ξ_i} \order[1]{ξ_j}\\
    ={} & \dot{\E}₂(v_i, v_j) \order[1]{ξ_i} \order[2]{ξ_j} + \dot{\E}₂(v_i, w_{j k}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} + \order[1]λ [\dot{\E}₂(v_i, w_j) + \dot{\E}₂(v_j, w_i)] \order[1]{ξ_i} \order[1]{ξ_j}.
  \end{aligned}
\end{equation*}
...
%       + 2 \order[1]λ \dot{\E}₂(v_{i}, w_{jk}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k}\bigr]\\
%     & -3\bigl\{ \E₃(v_i, v_j, w_{k l}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l} + 4 \order[1]λ \dot{\E}₂ (v_i, w_{j k}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k}\\
%     & + 2 ( \order[1]λ )^2 \bigl[\dot{\E}₂(v_i, w_j) + \dot{\E}₂(v_j, w_i)\bigr] \order[1]{ξ_i} \order[1]{ξ_j} \bigr\}\\
%     & + 4 \order[1]λ \dot{\E}₃(v_i, v_j, v_k) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k}\\
%     & + 12 \order[1]λ \bigl\{ \dot{\E}₂(v_i, v_j) \order[1]{ξ_i} \order[2]{ξ_j} + \dot{\E}₂(v_i, w_{j k}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} + \order[1]λ \bigl[\dot{\E}₂(v_i, w_j) + \dot{\E}₂(v_j, w_i)\bigr] \order[1]{ξ_i} \order[1]{ξ_j} \bigr\}\\
%     & + 6( \order[1]λ )^2 \ddot{\E}₂(v_i, v_j) \order[1]{ξ_i} \order[1]{ξ_j}\\
%     & + 6\order[2]λ \dot{\E}₂(v_i, v_j) \order[1]{ξ_i} \order[1]{ξ_j}.
%   \end{aligned}
% \end{equation*}
% \begin{equation*}
%   \begin{aligned}[b]
...
%     & + 12 \order[1]λ \dot{\E}₂(v_{i}, w_{jk}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} - 3 \E₃(v_i, v_j, w_{k l}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k} \order[1]{ξ_l} -12 \order[1]λ \dot{\E}₂ (v_i, w_{j k}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k}\\
%     & - 6 ( \order[1]λ )^2 \bigl[\dot{\E}₂(v_i, w_j) + \dot{\E}₂(v_j, w_i)\bigr] \order[1]{ξ_i} \order[1]{ξ_j}
%       + 4 \order[1]λ \dot{\E}₃(v_i, v_j, v_k) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k}\\
%     & + 12 \order[1]λ \dot{\E}₂(v_i, v_j) \order[1]{ξ_i} \order[2]{ξ_j} + 12 \order[1]λ \dot{\E}₂(v_i, w_{j k}) \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k}\\
%     & + 12 ( \order[1]λ )^2 \bigl[\dot{\E}₂(v_i, w_j)
...
%     & + \bigl[ 4 \order[1]λ \dot{\E}₃(v_i, v_j, v_k) + 12 \order[1]λ  \dot{\E}₂(v_i, w_{j k}) \bigr] \order[1]{ξ_i} \order[1]{ξ_j} \order[1]{ξ_k}\\
%     & + \bigl\{ 6( \order[1]λ )^2 \ddot{\E}₂(v_i, v_j) + 6( \order[1]λ )^2 \bigl[\dot{\E}₂(v_i, w_j) + \dot{\E}₂(v_j, w_i)\bigr] + 6\order[2]λ \dot{\E}₂(v_i, v_j) \bigr\} \order[1]{ξ_i} \order[1]{ξ_j}\\
%     & + \bigl[ 6\E₃(v_i, v_j, v_k) \order[1]{ξ_i} \order[1]{ξ_j} + 12 \order[1]λ \dot{\E}₂(v_j, v_k) \order[1]{ξ_j} \bigr] \order[2]{ξ_k}
%   \end{aligned}
% \end{equation*}
\begin{equation*}
  \begin{aligned}[b]
...
$f(η) = F [η, λ₀ + Λ(η)]$, with
\begin{equation*}
  F(η, λ) = \E_{, u u} [u^{\ast}(λ) + U(η), λ; \hat{u}, \hat{v}].
\end{equation*}
...
$F(0, λ) = \E₂(λ; \hat{u}, \hat{v})$ and, upon derivation with respect to $λ$
\begin{equation*}
  ∂_{λ} F(0, λ) = \dot{\E}₂(λ; \hat{u}, \hat{v})
  \quad \text{and} \quad
  ∂_{λλ}² F(0, λ) = \ddot{\E}₂(λ; \hat{u}, \hat{v}).
\end{equation*}

Successive differentiation of the definition of $F$ with respect to $η$ also
leads to
\begin{gather*}
  ∂_{η} F(η, λ) = \E_{, uuu}(U', \hat{u}, \hat{v}),\\
  ∂_{ηη}² F(η, λ) = \E_{, uuuu}(U', U', \hat{u}, \hat{v}) + \E_{, uuu}(U'', \hat{u}, \hat{v}),
\end{gather*}
where the differentials of $\E$ are evaluated at $[u^{\ast}(λ) + U(η), λ]$,
while the dérivatives of $U$ are evaluated at $y$. At $η = 0$, the above
relations read
\begin{gather*}
  ∂_{η} F(0, λ) = \E₃(λ; \order[1]u, \hat{u}, \hat{v}),\\
  ∂_{ηη}² F(0, λ) = \E₄(λ ; \order[1]u, \order[1]u, \hat{u}, \hat{v}) + \E₃(λ; \order[2]u, \hat{u}, \hat{v}),
\end{gather*}
and, upon differentiation with respect to $λ$
\begin{equation*}
  ∂_{η λ}² F(0, λ) = \dot{\E}₃(λ; \order[1]u, \hat{u}, \hat{v}).
\end{equation*}
...
derivatives of $f$
\begin{gather*}
  f'(0) = \E₃(λ₀; \order[1]u, \hat{u}, \hat{v}) + \order[1]λ \dot{\E}₂(λ₀; \hat{u}, \hat{v}),\\
  \begin{aligned}[b]
    f''(0) = {} & \E₄(λ₀; \order[1]u, \order[1]u, \hat{u}, \hat{v}) + \E₃(λ₀; \order[2]u, \hat{u}, \hat{v}) + 2\order[1]λ \dot{\E}₃(λ₀; \order[1]u, \hat{u}, \hat{v})\\
                & + ( \order[1]λ )^2 \ddot{\E}₂(λ₀; \hat{u}, \hat{v}) + \order[2]λ \dot{\E}₂(λ₀; \hat{u}, \hat{v}).
  \end{aligned}
\end{gather*}
...
\begin{equation*}
  \begin{aligned}[b]
    \E_{, uu} [u(η), λ(η); x, \hat{u}] ={}
    & \E₂(\order[0]x, \hat{u}) + η \bigl[ \E₂(\order[1]x, \hat{u}) + \E₃(\order[1]u, \order[0]x, \hat{u}) + \order[1]λ \dot{\E}₂(\order[0]x, \hat{u})\bigr]\\
    & + \tfrac{1}{2} η² \bigl[\E₂(\order[2]x, \hat{u}) + 2\E₃(\order[1]u, \order[1]x, \hat{u}) + 2 \order[1]λ \dot{\E}₂(\order[1]x, \hat{u})\\
    & + \E₄(\order[1]u, \order[1]u, \order[0]x, \hat{u}) + \E₃(\order[2]u, \order[0]x, \hat{u}) + 2\order[1]λ \dot{\E}₃(\order[1]u, \order[0]x, \hat{u})\\
    & + ( \order[1]λ )^2 \ddot{\E}₂(\order[0]x, \hat{u}) + \order[2]λ \dot{\E}₂(\order[0]x, \hat{u}) \bigr] + o(η²)
  \end{aligned}
\end{equation*}
(where the $\E_k$ and $\dot{\E}_k$ are all evaluated at $λ=λ₀$) and into the
...
\begin{equation*}
  \E₂(λ₀; \order[0]x, \hat{u}) = \order[0]α 〈 \order[0]x, \hat{u} 〉.
\end{equation*}

The above equation shows that $(\order[0]α, \order[0]x)$ is an eigenpair of $\E₂(λ₀)$. As
...
\begin{equation}
  ...
  \E₂(λ₀; \order[1]x, \hat{u}) + \E₃(λ₀; \order[1]u, \order[0]x, \hat{u}) + \order[1]λ \dot{\E}₂(λ₀; \order[0]x, \hat{u})
...
\begin{equation}
  ...
  \E₂(λ₀; \order[1]x, \hat{u}) + \E₃(λ₀; v_j, v_k, \hat{u}) \order[0]{χ_j} \order[1]{ξ_k} + \order[1]λ \dot{\E}₂(λ₀; v_j, \hat{u}) \order[0]{χ_j}
...
\begin{equation}
  \bigl[\E₃(λ₀; v_i, v_j, v_k) \order[1]{ξ_k} + \order[1]λ \dot{\E}₂(λ₀; v_i, v_j)\bigr] \order[0]{χ_j} = \order[1]α \order[0]{χ_i},
\end{equation}
...
\begin{equation}
  \E₂(\order[1]y, \hat{w}) + \E₃(v_i, v_j, \hat{w}) \order[0]{χ_i} \order[1]{ξ_j} + \order[1]λ \dot{\E}₂(v_i, \hat{w}) \order[0]{χ_i} = 0,
\end{equation}
...
\begin{multline*}
    \E₂(λ₀; \order[2]x, \hat{u})
    + 2\E₃(λ₀; \order[1]u, \order[1]x, \hat{u})
    + 2 \order[1]λ \dot{\E}₂(λ₀; \order[1]x, \hat{u})\\
    + \E₄(λ₀; \order[1]u, \order[1]u, \order[0]x, \hat{u})
    + \E₃(λ₀; \order[2]u, \order[0]x, \hat{u})
    + 2\order[1]λ \dot{\E}₃(λ₀; \order[1]u, \order[0]x, \hat{u})\\
    + ( \order[1]λ )^2 \ddot{\E}₂(λ₀; \order[0]x, \hat{u})
    + \order[2]λ \dot{\E}₂(λ₀; \order[0]x, \hat{u})
...
%   + 2 \order[1]λ \dot{\E}₂(v_i, \order[1]x)
%   + \E₄(v_i, \order[0]x, \order[1]u, \order[1]u)\\
%   + \E₃(v_i, \order[0]x, \order[2]u)
%   + 2\order[1]λ \dot{\E}₃(v_i, \order[0]x, \order[1]u)
%   + ( \order[1]λ )^2 \ddot{\E}₂(v_i, \order[0]x)\\
%   + \order[2]λ \dot{\E}₂(v_i, \order[0]x)
%   = 2\order[1]α 〈 v_i, \order[1]x 〉
%   + \order[2]α 〈 v_i, \order[0]x 〉,
% \end{multline*}
% \begin{multline*}
%   2\E₃(v_i, \order[1]{χ_i}v_j + \order[0]{χ_j}\order[1]{ξ_k}w_{jk}+ \order[1]λ \order[0]{χ_j} w_j, \order[1]{ξ_l} v_l)
%   + 2 \order[1]λ \dot{\E}₂(v_i, \order[1]{χ_i}v_j + \order[0]{χ_j}\order[1]{ξ_k}w_{jk} + \order[1]λ \order[0]{χ_j} w_j)\\
%   + \E₄(v_i, \order[0]{χ_j} v_j, \order[1]{ξ_k} v_k, \order[1]{ξ_l} v_l)
%   + \E₃(v_i, \order[0]{χ_j} v_j, \order[2]{ξ_k} v_k + \order[1]{ξ_k} \order[1]{ξ_l} w_{kl} + 2\order[1]λ \order[1]{ξ_k} w_k)\\
%   + 2\order[1]λ \dot{\E}₃(v_i, \order[0]{χ_j} v_j, \order[1]{ξ_k} v_k)
%   + ( \order[1]λ )^2 \ddot{\E}₂(v_i, \order[0]{χ_j} v_j) + \order[2]λ \dot{\E}₂(v_i, \order[0]{χ_j} v_j)\\
...
%   + 2 \order[1]λ \E₃(v_i,  w_j, v_k) \order[0]{χ_j} \order[1]{ξ_k}\\
%   + 2 \order[1]λ \dot{\E}₂(v_i, v_j) \order[1]{χ_i}
%   + 2 \order[1]λ \dot{\E}₂(v_i, w_{jk}) \order[0]{χ_j} \order[1]{ξ_k}
%   + ( \order[1]λ )^2 \dot{\E}₂(v_i, w_j) \order[0]{χ_j}\\
...
%   + 2\order[1]λ \E₃(v_i, v_j, w_k) \order[0]{χ_j} \order[1]{ξ_k}
%   + 2\order[1]λ \dot{\E}₃(v_i, v_j,  v_k) \order[0]{χ_j} \order[1]{ξ_k}
%   + ( \order[1]λ )^2 \ddot{\E}₂(v_i, v_j) \order[0]{χ_j}\\
%   + \order[2]λ \dot{\E}₂(v_i, v_j) \order[0]{χ_j} = 2\order[1]α\order[1]{χ_i} + \order[2]α \order[0]{χ_i}.
% \end{multline*}
\begin{multline*}
  \bigl[ \E₄(v_i, v_j,  v_k, v_l) + 2\E₃(v_i, w_{jk}, v_l) + \E₃(v_i, v_j, w_{kl})\bigr] \order[0]{χ_j} \order[1]{ξ_k} \order[1]{ξ_l}\\
  + 2 \order[1]λ \bigl[ \E₃(v_i,  w_j, v_k) + \dot{\E}₂(v_i, w_{jk}) + \E₃(v_i, v_j, w_k) + \dot{\E}₃(v_i, v_j,  v_k) \bigr] \order[0]{χ_j} \order[1]{ξ_k}\\
  + ( \order[1]λ )^2 \bigl[\dot{\E}₂(v_i, w_j) + \ddot{\E}₂(v_i, v_j)\bigr] \order[0]{χ_j} + \bigl[\E₃(v_i, v_j, v_k) \order[2]{ξ_k} + \order[2]λ \dot{\E}₂(v_i, v_j)\bigr] \order[0]{χ_j} \\
  +2\bigl[\E₃(v_i, v_j,  v_k)  \order[1]{ξ_k} + \order[1]λ \dot{\E}₂(v_i, v_j)\bigr] \order[1]{χ_i} = 2\order[1]α\order[1]{χ_i} + \order[2]α \order[0]{χ_i}.
\end{multline*}
...
\begin{multline*}
  ...
  + 2\order[1]λ \bigl[ -\E₂(w_{ik},  w_j) - \E₂(w_i, w_{jk}) - \E₂(w_{ij}, w_k) + \dot{\E}₃(v_i, v_j,  v_k) \bigr] \order[0]{χ_j} \order[1]{ξ_k}\\
  + ( \order[1]λ )^2 \bigl[\dot{\E}₂(v_i, w_j) + \ddot{\E}₂(v_i, v_j)\bigr] \order[0]{χ_j}
  + \bigl[\E₃(v_i, v_j, v_k) \order[2]{ξ_k} + \order[2]λ \dot{\E}₂(v_i, v_j)\bigr] \order[0]{χ_j} \\
  +2\bigl[\E₃(v_i, v_j,  v_k)  \order[1]{ξ_k}
  + \order[1]λ \dot{\E}₂(v_i, v_j)\bigr] \order[1]{χ_i}
