...
\begin{equation}
  \begin{aligned}[b]
    Δℰ ={} &  ℰ(u₀ + ξ \× v + η \× w, λ₀) - ℰ(u₀, λ₀)\\
    ={} & \tfrac{1}{2} ℰ₂(ξ \× v + η \× w, ξ \× v + η \× w) + \tfrac{1}{6} ℰ₃(ξ \× v + η \× w, ξ \× v + η \× w, ξ \× v + η \× w)\\
    & + \tfrac{1}{24} ℰ₄(ξ \× v + η \× w, ξ \× v + η \× w, ξ \× v + η \× w, ξ \× v + η \× w) + \mathcal{O}\bigl[\bigl(ξ^2 + η^2\bigr)^2\bigr],
  \end{aligned}
\end{equation}
...
\end{equation}
pour tous \(ξ^i, ξ^j, ξ^k, ξ^l∈\reals\). On peut montrer que l'inégalité
stricte est une condition \emph{suffisante} de stabilité.
...
\begin{proof}
 Soient \(u∈\ker \mathcal{B}\), \(v∈V\) et \(α∈\reals\). Comme  précédemment, on écrit que \(\mathcal{B}(w, w) \geq 0\), avec \(w  = α u + v\)
 \begin{equation}
