...
point. On sait que \(ℰ_{,uu}(u₀)\geq0\) est une condition
...
\(u₀+ξ v+\eta w\) voisin du point d'équilibre \(u₀\), avec
\(ξ, \eta\in\reals\) \guillemotleft{}petits\guillemotright{}, \(v\in V\) and
\(w\in W\). On obtient alors, à l'ordre 4 en \(ξ\) et \(\eta\)
\begin{equation}
  \begin{aligned}[b]
    \Deltaℰ ={}&
    ℰ(u₀+ξ v+\eta w)-ℰ(u₀)\\
    ={}&\tfrac12ℰ_{,uu}(u₀;ξ v+\eta w, ξ v+\eta w)
    +\tfrac16ℰ_{,uuu}(u₀;ξ v+\eta w, ξ v+\eta w, ξ v+\eta w)\\
    &+\tfrac1{24}ℰ_{,uuuu}(u₀;ξ v+\eta w, ξ v+\eta w, ξ v+\eta w,
    ξ v+\eta w)+\LandauO\bigl(\bigl(ξ^2+\eta^2\bigr)^2\bigr),
  \end{aligned}
\end{equation}
...
\begin{equation}
  \begin{aligned}[b]
    \Deltaℰ
    ={}&\tfrac12\eta^2ℰ_{,uu}(w, w)
    +\tfrac16ξ^3ℰ_{,uuu}(v, v, v)
    +\tfrac12ξ^2\etaℰ_{,uuu}(v, v, w)\\
    &+\tfrac12ξ\eta^2ℰ_{,uuu}(v, w, w)
    +\tfrac16\eta^3ℰ_{,uuu}(w, w, w)\\
    &+\tfrac1{24}ξ^4ℰ_{,uuuu}(v, v, v, v)
    +\tfrac16ξ^3ℰ_{,uuuu}(v, v, v, w)\\
    &+\tfrac14ξ^2\eta^2ℰ_{,uuuu}(v, v, w, w)
    +\tfrac16ξ\eta^3ℰ_{,uuuu}(v, w, w, w)\\
    &+\tfrac1{24}\eta^4ℰ_{,uuuu}(w, w, w, w)
    +\LandauO\bigl(\bigl(ξ^2+\eta^2\bigr)^2\bigr),
  \end{aligned}
\end{equation}
...
pour tous \(ξ\) et \(\eta\) suffisamment petits. En prenant tout d'abord
\(\eta=0\), on obtient les conditions nécessaires
\begin{equation}
  \label{eq:20211108164416}
  ℰ_{,uuu}(v, v, v)=0
  \quad\text{et}\quad
  ℰ_{,uuuu}(v, v, v, v)\geq0
...
\(ℰ_{,u}[u(\eta), \lambda(\eta)]\) est nul. Le développement limité du
...
\begin{equation}
  ...
  \begin{aligned}[b]
    ℰ_{,uu}[u(\eta), \lambda(\eta)](\hat{u}, \hat{v})
...
Soit \(f(\eta)=ℰ_{,u}(u, \lambda)(\hat{u})\) le \emph{résidu}
...
\begin{equation}
  F(\eta, \lambda)=ℰ_{,u}[u₀(\lambda)+U(\eta), \lambda](\hat{u})
\end{equation}
...
\(h(\eta)=ℰ_{,uu}(u, \lambda)(\hat{u}, \hat{v})\), où \(\lambda\) et
...
\begin{equation}
  H(\eta, \lambda)=ℰ_{,uu}[u₀(\lambda)+U(\eta), \lambda],
\end{equation}
de sorte que
\begin{equation}
...
\begin{equation}
  ...
  \partial_\eta H(\eta, \lambda)=ℰ_{,uuu}(u₀+U, \lambda)(U', \hat{u}, \hat{v}),
\end{equation}
soit, en \(\eta=0\)
\begin{equation}
...
\begin{equation}
  ...
  ℰ_{,uu}[u(\eta), \lambda(\eta)](v, \hat{u})
  =\alpha〈 v, \hat{u}〉
...
\begin{equation}
  \begin{aligned}[b]
    ℰ_{,uu}[u(\eta), \lambda(\eta)](v, \hat{w})={}
...
\begin{equation}
  ...
  +\eta\bigl(\alpha_1〈 v_0, \hat{w}〉+\alpha_0〈 v_1, \hat{w}〉\bigr)
  +o(\eta).
\end{equation}
