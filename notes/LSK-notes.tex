...
$u = \xi v + η w$, with $\xi, η \in \reals$ and $v \in V$ and $w \in W$ are fixed,
...
\begin{equation}
  \begin{aligned}[b]
    \E(u_0 + u, \lambda_0) - \E(u_0, \lambda_0) ={}
    & \tfrac{1}{2} η^2 \E_2(\lambda_0; w, w) + \tfrac{1}{6} \xi^3 \E_3(\lambda_0; v, v, v)\\
    & + \tfrac{1}{2} \xi^2 η \E_3(\lambda_0; v, v, w) + \tfrac{1}{2} \xi η^2 \E_3(\lambda_0; v, w, w)\\
    & + \tfrac{1}{6} η^3 \E_3(\lambda_0; w, w, w) + \tfrac{1}{24} \xi^4 \E_4(\lambda_0; v, v, v, v)\\
    & + \tfrac{1}{6} \xi^3 η \E_4(\lambda_0; v, v, v, w) + \tfrac{1}{4} \xi^2 η^2 \E_4(\lambda_0; v, v, w, w)\\
    & + \tfrac{1}{6} \xi η^3 \E_4(\lambda_0; v, w, w, w) + \tfrac{1}{24} η^4 \E_4(\lambda_0; w, w, w, w)\\
    & + o\bigl[\bigl(\xi^2 + η^2\bigr)^2\bigr].
  \end{aligned}
\end{equation}

For the equilibrium to be stable, the above expression must be $≥ 0$ for all
$\xi$ et $η$ small enough. Taking first $η = 0$, we get the following necessary conditions
\begin{equation}
...
conditions~\eqref{eq:20211108164416} to hold, we now take $η = \xi^2$
\begin{equation}
  \begin{aligned}[b]
...
$\xi_1$, \dots, $\xi_m$. Finally, a parametrization $η$ of $\xi_1$, \dots
...
\begin{equation}
  ...
  E_{ijk} +  F_{jk} \frac{∂\lambda}{∂\xi_i} \biggr\rvert_{\xi_1 = \cdots = \xi_m = 0} + F_{ik} \frac{∂\lambda}{∂\xi_j} \biggr\rvert_{\xi_1 = \cdots = \xi_m = 0} = 0.
\end{equation}
...
\begin{equation}
  \order[1]{\lambda} = \order[1]{\xi_i} \frac{∂\lambda}{∂\xi_i} \biggr\rvert_{\xi_1 = \cdots = \xi_m = 0}
\end{equation}
...
\begin{equation}
  \begin{aligned}[b]
    0 &= E_{ijk} \order[1]{\xi_i} \order[1]{\xi_j} +  F_{jk} \order[1]{\xi_i} \order[1]{\xi_j} \frac{∂\lambda}{∂\xi_i} \biggr\rvert_{\xi_1 = \cdots = \xi_m = 0} + F_{ik} \order[1]{\xi_i} \order[1]{\xi_j} \frac{∂\lambda}{∂\xi_j} \biggr\rvert_{\xi_1 = \cdots = \xi_m = 0}\\
...
\begin{equation}
  w'(η) = w_{,i} \xi_i' + w_{,\lambda} \lambda'
  \quad \text{and} \quad
  w''(η) = w_{,ij} \xi_i' \xi_j' + 2 w_{,i\lambda} \xi_i' \lambda' + w_{,i} \xi_i'' + w_{,\lambda\lambda} \lambda^{'2} + w_{,\lambda} \lambda''
\end{equation}
and, at $η = 0$
\begin{equation}
...
\begin{equation}
  u(η) = u^\ast[\lambda(η)] + \order[1]{\xi_i} v_i + \tfrac{1}{2} \bigl( \order[2]{\xi_i} v_i + \order[1]{\xi_i} \order[1]{\xi_j} w_{ij}  + 2\order[1]{\lambda} \order[1]{\xi_i} w_i\bigr) + o(η^2),
\end{equation}
...
\begin{equation}
  ...
  \begin{aligned}[b]
    \E[u(η), \lambda(η)] ={} & \E\{u^{\ast}[\lambda(η)], \lambda(η)\} + \tfrac{1}{6} \order[1]\lambda η^3 F_{i j} \order[1]{\xi_i} \order[1]{\xi_j} + \tfrac{1}{24} η^4 \bigl\{E_{ijkl} \order[1]{\xi_i} \order[1]{\xi_j} \order[1]{\xi_k} \order[1]{\xi_l}\\
    & + 4\order[1]\lambda \mathring{E}_{ijk} \order[1]{\xi_i} \order[1]{\xi_j} \order[1]{\xi_k} + 6 \bigl[( \order[1]\lambda )^2 \mathring{F}_{ij} + \order[2]\lambda F_{ij}\bigr] \order[1]{\xi_i} \order[1]{\xi_j}\bigr\} + o(η^4).
  \end{aligned}
\end{equation}
...
\begin{equation}
  \E[u(η), \lambda(η)] = \E\{u^{\ast}[\lambda(η)], \lambda(η)\} + \tfrac{1}{6} \order[1]\lambda η^3 F_{i j} \order[1]{\xi_i} \order[1]{\xi_j} + o(η^3).
\end{equation}
...
\begin{equation}
  ...
  \begin{aligned}[b]
    \E[u(η), \lambda(η)]
    &= \E\{u^{\ast}[\lambda(η)], \lambda(η)\} + \tfrac{1}{24} η^4 \bigl(E_{ijkl} \order[1]{\xi_i} \order[1]{\xi_j} \order[1]{\xi_k} \order[1]{\xi_l}  + 6  \order[2]\lambda F_{ij} \order[1]{\xi_i} \order[1]{\xi_j}\bigr) + o(η^4)\\
    &= \E\{u^{\ast}[\lambda(η)], \lambda(η)\} - \tfrac{1}{24} η^4 E_{ijkl} \order[1]{\xi_i} \order[1]{\xi_j} \order[1]{\xi_k} \order[1]{\xi_l} + o(η^4).
  \end{aligned}
\end{equation}

The expansion $\lambda = \lambda_0 + \order[2]\lambda η^2 / 2 + o(η^2)$ can be inverted as follows
\begin{equation}
  η^4 = \frac{4 \bigl(\lambda - \lambda_0\bigr)^2}{\bigl( \order[2]\lambda \bigr)^2} + o(\lambda^2) = \frac{36 \bigl( F_{ij} \order[1]{\xi_i} \order[1]{\xi_j} \bigr)^2}{\bigl( E_{ijkl} \order[1]{\xi_i} \order[1]{\xi_j} \order[1]{\xi_k} \order[1]{\xi_l} \bigr)^2} \bigl( \lambda - \lambda_0 \bigr)^2
\end{equation}
and expression~\eqref{eq:20220801094437} reads
\begin{equation}
  \E[u(η), \lambda(η)] = \E\{u^{\ast}[\lambda(η)], \lambda(η)\} - \frac{3 \bigl( F_{ij} \order[1]{\xi_i} \order[1]{\xi_j} \bigr)^2}{2 E_{ijkl} \order[1]{\xi_i} \order[1]{\xi_j} \order[1]{\xi_k} \order[1]{\xi_l} } \bigl( \lambda - \lambda_0 \bigr)^2  + o(\lambda^2).
\end{equation}
...
\begin{equation}
  \begin{aligned}[b]
    \E_{, uu}[u(η), \lambda(η), v_1, v_1] &= η \bigl(E_{111} \order[1]{\xi_1} + \order[1]\lambda F_{11}\bigr) + o(η) = -2 η F_{11} + o(η)\\
...
\begin{equation*}
  \E_{, uu}[u(η), \lambda(η); v_1, v_1] = \tfrac{1}{2} η^2 \bigl[ E_{1111}\bigl(\xi_1^1\bigr) ^2 + \order[2]\lambda F_{11} \bigr] + o(η^2) = - η^2 \order[2]\lambda F_{11} + o(η^2),
\end{equation*}
