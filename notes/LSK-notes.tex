...
\begin{equation}
  ...
  \partial_{η\lambda}²F(0, \lambda)=\dot{ℋ}(u₁, \hat{u}).
\end{equation}
...
\begin{equation}
  \bigl(\mathcal T_{ijk}^\astξ_k+\lambda₁\dot{ℋ}_{ij}^\ast\bigr)\chi_j
  =\alpha₁\chi_i,
\end{equation}
qui est un problème aux valeurs propres pour la matrice symétrique
\((\mathcal T_{ijk}^\astξ_k+\lambda₁\dot{ℋ}_{ij}^\ast)_{1≤i,
  j≤m}\)
