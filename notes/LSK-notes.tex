...
branch is a curve $(u, \lambda) \in ℝ ^ {m + 1}$, which is parametrized by $\eta$:
...
\begin{gather*}
  f'(\eta) = ∂_{\eta} F + \Lambda' ∂_{\lambda} F,\\
  f''(\eta) = ∂_{\eta\eta}^2 F + 2\Lambda' ∂_{\eta\lambda}^2F + \Lambda'^2 ∂_{\lambda\lambda}^2 F + \Lambda'' ∂_{\lambda} F,\\
  \begin{aligned}[b]
    f'''(\eta) ={}
    & ∂_{\eta\eta\eta}^3 F + 3\Lambda' ∂_{\eta\eta\lambda}^3F + 3\Lambda'^2 ∂_{\eta\lambda\lambda}^3F + \lambda'^3 ∂_{\lambda\lambda\lambda}^3 F\\
    & + 3\Lambda'' ∂_{\eta\lambda}^2 F + 3\Lambda' \Lambda'' ∂_{\lambda \lambda}^2 F + \Lambda''' ∂_{\lambda} F,
  \end{aligned}\\
  \begin{aligned}[b]
    f''''(\eta) ={}
    & ∂_{\eta\eta\eta\eta}^4 F + 4\Lambda' ∂_{\eta\eta\eta\lambda}^4F + 6\Lambda'^2 ∂_{\eta\eta\lambda\lambda}^4F + 4\Lambda'^3 ∂_{\eta\lambda\lambda\lambda}^4F + \Lambda'^4 ∂_{\lambda\lambda\lambda\lambda}^4 F\\
    & + 6\Lambda'' ∂_{\eta\eta\lambda}^3 F + 12\Lambda' \Lambda'' ∂_{\eta\lambda\lambda}^3F + 6\Lambda'^2 \Lambda'' ∂_{\lambda\lambda\lambda}^3 F\\
    & + 4 \Lambda''' ∂_{\eta\lambda}^2 F + \bigl( 3\Lambda''^2 + 4 \Lambda' \Lambda''' \bigr) ∂_{\lambda\lambda}^2 F + \lambda'''' ∂_{\lambda}F,
  \end{aligned}
\end{gather*}
...
\begin{gather}
  ...
  f'(0) = ∂_{\eta} F + \order[1]\lambda ∂_{\lambda} F,\\
  \label{eq:20220107124311}
  f''(0) = ∂_{\eta\eta}^2 F + 2 \order[1]\lambda ∂_{\eta\lambda}^2 F + \bigl( \order[1]\lambda \bigr)^2 ∂_{\lambda\lambda}^2 F + \order[2]\lambda ∂_{\lambda} F,\\
  \label{eq:20220107060500}
  \begin{aligned}[b]
    f'''(0) ={}
    & ∂_{\eta\eta\eta}^3 F + 3 \order[1]\lambda ∂_{\eta\eta\lambda}^3 F + 3 \bigl( \order[1]\lambda \bigr)^2 ∂_{\eta\lambda\lambda}^3 F + \bigl( \order[1]\lambda \bigr)^3 ∂_{\lambda\lambda\lambda}^3 F\\
    & + 3 \order[2]\lambda ∂_{\eta\lambda}^2 F + 3 \order[1]\lambda \order[2]\lambda ∂_{\lambda\lambda}^2 F + \order[3]\lambda ∂_{\lambda} F,
  \end{aligned}\\
  \label{eq:20220602185935}
  \begin{aligned}[b]
    f''''(0) ={}
    & ∂_{\eta\eta\eta\eta}^4F + 4 \order[1]\lambda ∂_{\eta\eta\eta\lambda}^4 F + 6 \bigl( \order[1]\lambda \bigr)^2 ∂_{\eta\eta\lambda\lambda}^4 F + 4 \bigl( \order[1]\lambda \bigr)^3 ∂_{\eta\lambda\lambda\lambda}^4 F + \bigl( \order[1]\lambda \bigr)^4 ∂_{\lambda\lambda\lambda\lambda}^4 F\\
    & + 6 \order[2]\lambda ∂_{\eta\eta\lambda}^3 F + 12 \order[1]\lambda \order[2]\lambda ∂_{\eta\lambda\lambda}^3 F + 6 \bigl( \order[1]\lambda \bigr)^2 \order[2]\lambda ∂_{\lambda\lambda\lambda}^3 F\\
    & + 4 \order[3]\lambda ∂_{\eta\lambda}^2 F + \bigl(3 \bigl( \order[2]\lambda \bigr)^2 + 4 \order[1]\lambda \order[3]\lambda\bigr) ∂_{\lambda\lambda}^2 F + \lambda_4 ∂_{\lambda} F,
  \end{aligned}
\end{gather}
...
to $\lambda$, we get
\begin{equation*}
  \frac{∂^k F}{∂ \lambda^k}(0, \lambda) = 0 \quad \text{for all} \quad k \geq 0.
\end{equation*}
From the definition~\eqref{eq:20220107054629} of $F$, we find successively
\begin{equation*}
  ∂_{\eta}F(\eta, \lambda) = \E_{, u u}[u^{\ast}(\lambda) + U(\eta), \lambda; U'(\eta), \hat{u}],
\end{equation*}
\begin{equation*}
  \begin{aligned}[b]
    ∂_{\eta \eta}^2 F(\eta, \lambda) ={}
...
and, at $\eta = 0$
\begin{gather*}
  ∂_{\eta}F(0, \lambda) = \E_2(\lambda; \order[1]u, \hat{u}),\\
  ∂_{\eta\eta}^2 F(0, \lambda) = \E_3(\lambda; \order[1]u, \order[1]u, \hat{u}) + \E_2(\lambda; \order[2]u, \hat{u}),\\
  ∂_{\eta\eta\eta}^3 F(0, \lambda) = \E_4(\lambda; \order[1]u, \order[1]u, \order[1]u, \hat{u}) + 3\E_3(\lambda; \order[1]u, \order[2]u, \hat{u}) + \E_2(\lambda; \order[3]u, \hat{u}).
\end{gather*}
Upon derivation with respect to $\lambda$, we find successively
\begin{gather*}
  ∂_{\eta\lambda}^2 F(0, \lambda) = \dot{\E}_2(\lambda; \order[1]u, \hat{u}),\\
  ∂_{\eta\lambda\lambda}^3 F(0, \lambda) = \ddot{\E}_2(\lambda; \order[1]u, \hat{u}),\\
  ∂_{\eta\eta\lambda}^3 F(0, \lambda) = \dot{\E}_3(\lambda; \order[1]u, \order[1]u, \hat{u}) + \dot{\E_2}(\lambda; \order[2]u, \hat{u}).
\end{gather*}
...
\begin{gather*}
  ∂_{\eta} F(\eta, \lambda) = \E_{, u}(U'),\\
  ∂_{\eta\eta}^2 F(\eta, \lambda) = \E_{, uu} (U', U') + \E_{, u} (U''),\\
  ∂_{\eta\eta\eta}^3 F(\eta, \lambda) = \E_{, uuu}(U', U', U') + 3\E_{, uu}(U', U'') + \E_{, u}(U'''),\\
  \begin{aligned}[b]
    ∂_{\eta\eta\eta\eta}^4 F ={}
...
\begin{gather*}
  ∂_{\eta} F(0, \lambda) = 0,\\
  ∂_{\eta\eta}^2 F(0, \lambda) =\E_2(\lambda ; \order[1]u, \order[1]u),\\
  ∂_{\eta\eta\eta}^3 F(0, \lambda) = \E_3(\lambda; \order[1]u, \order[1]u, \order[1]u) + 3\E_2(\lambda; \order[1]u, \order[2]u),\\
  \begin{aligned}[b]
    ∂_{\eta\eta\eta\eta}^4 F(\eta, \lambda) ={} & \E_4(\lambda; \order[1]u, \order[1]u, \order[1]u, \order[1]u) + 6\E_3(\lambda; \order[1]u, \order[1]u, \order[2]u)\\
...
\begin{equation*}
  \begin{gathered}
    ∂_{\eta\lambda}^2 F(0, \lambda) = 0,\\
    ∂_{\eta\eta\lambda}^3 F(0, \lambda) = \dot{\E}_2(\lambda; \order[1]u, \order[1]u),\\
    ∂_{\eta\lambda\lambda}^3 F(0, \lambda) = 0,\\
  \end{gathered}
  \qquad
  \begin{gathered}
    ∂_{\eta\eta\eta\lambda}^4 F(0, \lambda) = \dot{\E}_3(\lambda; \order[1]u, \order[1]u, \order[1]u) + 3\dot{\E}_2(\lambda; \order[1]u, \order[2]u),\\
    ∂_{\eta\eta\lambda\lambda}^4 F(0, \lambda) = \ddot{\E}_2(\lambda; \order[1]u, \order[1]u),\\
    ∂_{\eta\lambda\lambda\lambda}^4 F(0, \lambda) = 0
  \end{gathered}
\end{equation*}
and finally
\begin{gather}
...
leads to
\begin{gather*}
  ∂_{\eta} F(\eta, \lambda) = \E_{, uuu}(U', \hat{u}, \hat{v}),\\
  ∂_{\eta\eta}^2 F(\eta, \lambda) = \E_{, uuuu}(U', U', \hat{u}, \hat{v}) + \E_{, uuu}(U'', \hat{u}, \hat{v}),
\end{gather*}
...
relations read
\begin{gather*}
  ∂_{\eta} F(0, \lambda) = \E_3(\lambda; \order[1]u, \hat{u}, \hat{v}),\\
  ∂_{\eta\eta}^2 F(0, \lambda) = \E_4(\lambda ; \order[1]u, \order[1]u, \hat{u}, \hat{v}) + \E_3(\lambda; \order[2]u, \hat{u}, \hat{v}),
\end{gather*}
and, upon differentiation with respect to $\lambda$
\begin{equation*}
  ∂_{\eta \lambda}^2 F(0, \lambda) = \dot{\E}_3(\lambda; \order[1]u, \hat{u}, \hat{v}).
\end{equation*}
