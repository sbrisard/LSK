...
When the tensor \(E_{ijk}\) is not identically null,
Eq.~\eqref{eq:20220524135036} has at most \((2^m - 1)\) pairs of real solutions
\((λ₁, u₁)\) et \((- λ₁, - u₁)\). \pdfmargincomment{Je ne sais pas démontrer ce
  résultat sur le nombre de solutions réelles.} Along each of the bifurcated
branches, we have
\begin{equation}
  λ = λ₀ + η λ₁ + o(η)
  \quad \text{and} \quad
...
\pdfmarkupcomment{Assuming that}{Est-ce vraiment une hypothèse ? À un
  changement de paramètre près, on doit toujours pouvoir se débrouiller pour que
  ce soit le cas.}  \(λ₁ ≠ 0\), \(η\) can be eliminated
\begin{equation}
  u = u_0 + \frac{λ - λ₀}{λ₁} ξ₁^i v_i + o(η).
\end{equation}
...
\end{multline*}
[where the \(\dot{ℰ}_k\) are all evaluated at \(λ=λ₀\)] and into the scalar product \(α 〈 x, \hat{u} 〉\) on the other hand
\begin{multline*}
