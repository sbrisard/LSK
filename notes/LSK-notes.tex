...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
et, en utilisant la définition~\eqref{eq:20211221155859} de \(w_{kl}\), avec
\(\hat{w}=w_{ij}\)
\begin{equation}
  ℰ₃(λ₀; w_{ij}, v_k, v_l)=-2ℰ₂(λ₀; w_{ij}, w_{kl}),
\end{equation}
soit
\begin{equation}
  \begin{aligned}[b]
    ℰ₃(λ₀; u₂, \hat{u}, \hat{u})={}&
    -2ξ₁^iξ₁^j\hat{ξ}^k\hat{ξ}^lℰ₂(λ₀; w_{ij}, w_{kl})
    +ξ₂^i\hat{ξ}^j\hat{ξ}^kℰ_{ijk}(λ₀)\\
    &+λ₁ξ₁^i\hat{ξ}^j\hat{ξ}^kℰ₃(λ₀; w_i, v_j, v_k).
  \end{aligned}
\end{equation}
