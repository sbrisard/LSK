...
clearer below. Eq.~\eqref{eq:20220825143616} is first derived with respect to
\(ξ^i\) and \(λ\)
\begin{gather}
  ℰ_{,uu}(u^\ast + ξ^i v_i + w, λ; v_i + w_{,i}, \hat{w}) = 0,\\
  ℰ_{,uu}(u^\ast + ξ^i v_i + w, λ; \dot{u}^\ast + w_{,λ}, \hat{w}) + ℰ_{,uλ}(u^\ast + ξ^i v_i + w, λ; \hat{w}) = 0.
\end{gather}
for all \(\hat{w} ∈ W\). At \(ξ^i = 0, λ = λ₀\), the above equations read
\begin{equation}
\label{eq:20220825150219}
  ℰ₂(λ₀; v_i + w_{,i}, \hat{w}) = ℰ₂(λ₀; v_i, \hat{w}) + ℰ₂(λ₀; w_{,i}, \hat{w}) = ℰ₂(λ₀; w_{,i}, \hat{w}) = 0
\end{equation}
and
\begin{equation}
  \begin{aligned}[b]
    ℰ_{,uu}(u₀, λ₀; w_{,λ}, \hat{w}) + ℰ_{,uu}(u₀, λ₀; \dot{u}₀, \hat{w}) + ℰ_{,uλ}(u₀, w₀, λ; \hat{w}) &=\\
    ℰ₂(λ₀; w_{,λ}, \hat{w}) + \dot{ℰ}₂(u₀, λ₀; \hat{w}) &= 0.
  \end{aligned}
\end{equation}

Note that in Eq.~\eqref{eq:20220825150219}, \(ℰ₂(λ₀; v_i, \hat{w}) = 0\) since
\(v_i ∈ V\). Eq.~\eqref{eq:20220825150219} holds for any \(\hat{w} ∈ W\). Since
\(w ∈ W\) for all \(ξ^i\) and \(λ\), we have \(w_{,i} ∈ W\) and
Eq.~\eqref{eq:20220825150219} holds for \(\hat{w} = w_{,i} ∈ W\)
\begin{equation}
  ℰ₂(λ₀; w_{,i}, w_{,i}) = 0 \quad \text{therefore} \quad w_{,i}(0, \ldots, 0, λ₀) = 0.
\end{equation}
