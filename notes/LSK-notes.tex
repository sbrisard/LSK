...
\begin{eqnarray}
  ...
  \nonumber\\
  &  & + \nobracket 6 [\mathlambda_1^2  \dot{F}_{i  j}
  (\mathlambda_0) + \lambda_2 F_{i  j} (\lambda_0)] \xi_1^i \xi_1^j \}
...
\begin{eqnarray*}
  ...
  \xi_1^k \xi_1^l + 4 \mathlambda_1  \dot{E}_{i  j  k}
  (\lambda_0) \xi_1^i \xi_1^j \xi_1^k + 6 [\mathlambda_1^2  \dot{F}_{i
   j} (\mathlambda_0) + \lambda_2 F_{i  j} (\lambda_0)]
  \xi_1^i \xi_1^j\\
  &  & \nosymbol + 6 [E_{i  j  k} (\lambda_0) \xi_1^k + 2
  \mathlambda_1 F_{i  j} (\lambda_0)] \xi_1^i \xi_2^j,
\end{eqnarray*}
...
\begin{equation}
  \label{eq:DL energie derivee 4ieme} f'''' (0) = E_{i  j  k
   l} (\lambda_0) \xi_1^i \xi_1^j \xi_1^k \xi_1^l + 4 \mathlambda_1
  \dot{E}_{i  j  k} (\lambda_0) \xi_1^i \xi_1^j \xi_1^k + 6
  [\mathlambda_1^2  \dot{F}_{i  j} (\mathlambda_0) + \lambda_2 F_{i
...
\begin{remark}
  ...
  \begin{eqnarray*}
    ...
    j} (\lambda_0) \xi_1^i \xi_1^j - 3 A_{i  j} (\mathlambda_0)
    \xi_1^i \xi_2^j - 3 \lambda_1  [\dot{E}_{i  j  k}
    (\mathlambda_0) \xi_1^k + \lambda_1  \dot{F}_{i  j} (\lambda_0)]
    \xi_1^i \xi_1^j\\
    & = & - 3 \lambda_1  \dot{E}_{i  j  k} (\mathlambda_0)
    \xi_1^i \xi_1^j \xi_1^k - 3 [\lambda_1^2  \dot{F}_{i  j}
    (\lambda_0) + \lambda_2 F_{i  j} (\lambda_0)] \xi_1^i \xi_1^j - 3
    A_{i  j} (\mathlambda_0) \xi_1^i \xi_2^j,
  \end{eqnarray*}
  soit
  \[ f'''' (0) = \mathlambda_1  \dot{E}_{i  j  k} (\lambda_0)
     \xi_1^i \xi_1^j \xi_1^k + 3 [\mathlambda_1^2  \dot{F}_{i  j}
     (\mathlambda_0) + \lambda_2 F_{i  j} (\lambda_0)] \xi_1^i \xi_1^j
     - 3 A_{i  j} (\mathlambda_0) \xi_1^i \xi_2^j . \]
\end{remark}
