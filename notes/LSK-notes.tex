...
\begin{equation}
  ...
  λ] .
\end{equation}
Noter que $ℰ₂$, $ℰ_3$ et $ℰ_4$ sont des formes
...
$ℰ$, ainsi que du fait que $ℰ₂ (v, \bullet) = 0$ (puisque
...
\begin{equation}
  \label{eq20211109145356} ℰ₂ (w, w) +ℰ_3 (v, v, w) +
...
variationnel
\begin{equation}
  \label{eq20211109145224} 2ℰ₂ (w, \hat{w}) +ℰ_3 (v, v,
...
s'écrit
\begin{equation}
  [ℰ_4 (v_i, v_j, v_k, v_l) - 3ℰ₂ (w_{i  j}, w_{k
...
$\dot{ℰ}₂ (λ₀)$ est bien définie négative}
\begin{equation}
...
\begin{eqnarray}
  ...
  = & ℰ₂ (λ₀ ; \hat{u}, \hat{v}) + η [ℰ_3
  (λ₀ ; u₁, \hat{u}, \hat{v})   + λ₁
  \dot{ℰ₂} (λ₀ ; \hat{u}, \hat{v})] \nonumber\\
  &  &  + \tfrac{1}{2} η^2  [ℰ_4 (λ₀ ; u₁, u₁,
  \hat{u}, \hat{v})  +ℰ_3 (λ₀ ; u₂, \hat{u},
...
\begin{equation}
  ...
  (λ₀)}{2 \dot{ℰ}_{1  1} (λ₀)},
\end{equation}
où on remarque que le quotient a un sens, puisque $\dot{ℰ₂}
...
\begin{equation}
  ...
  (λ - λ₀) .
\end{equation}
Comme $\dot{ℰ}₂ (λ₀)$ est définie négative, la
...
\begin{eqnarray}
  λ (η) & = & λ (η) - λ₀ = η λ₁ +
  \tfrac{1}{2} η^2 λ₂ + \tfrac{1}{6} η^3 λ_3 + \cdots,
  \label{eq20211112155446}\\
  U (η) & = & u (η) - u^{\ast} [λ (η)] = η u₁ +
  \tfrac{1}{2} η^2 u₂ + \tfrac{1}{6} η^3 u_3 + \cdots .
...
\begin{equation} f''' (0) =ℰ_3 (λ₀ ; u₁, u₁, u₁) + 3ℰ₂
   (λ₀ ; u₁, u₂) + 3 λ₁  \dot{ℰ}₂ (λ₀ ; u₁,
   u₁), \end{equation}
\begin{eqnarray}
  f'''' (0) & = & ℰ_4 (λ₀ ; u₁, u₁, u₁, u₁) +
...
on trouve tout d'abord, pour $ℰ_3 (λ₀ ; u₁, u₁, u₂)$
\begin{eqnarray*}
  ℰ_3 (λ₀ ; u₁, u₁, u₂) & = & ℰ_3 (λ₀ ;
  v_i, v_j, v_k) ξ₁^i ξ₁^j ξ₂^k +ℰ_3 (λ₀ ; v_i, v_j,
  w_{k  l}) ξ₁^i ξ₁^j ξ₁^k ξ₁^l\\
  &  &  + λ₁ ℰ_3 (λ₀ ; v_i, v_j, w_k)
  ξ₁^i ξ₁^j ξ₁^k\\
  & = & ℰ_3 (λ₀ ; v_i, v_j, v_k) ξ₁^i ξ₁^j ξ₂^k
...
On observe tout d'abord que $F (0, λ) =ℰ₂ (λ ; \hat{u},
...
\begin{eqnarray*}
  ...
  x₁, \hat{u} \rangle)\\
  &  & + \tfrac{1}{2} η^2  (α₂ \langle x₀, \hat{u} \rangle + 2
  α₁ \langle x₁, \hat{u} \rangle + α₀ \langle x₂, \hat{u}
  \rangle) + o (η^2) .
\end{eqnarray*}
\paragraph{Problème variationnel d'ordre 0}Trouver $x₀∈U$ et
$α₀∈\mathbb{R}$ tels que, pour tout $\hat{u}∈U$
\begin{equation} ℰ₂ (λ₀ ; x₀, \hat{u}) = α₀  \langle x₀, \hat{u}
   \rangle . \end{equation}
On en déduit que $x₀$ est le vecteur propre de $ℰ₂
(λ₀)$ associé à la valeur propre $α₀$. Si $α₀ \neq
0$, $ℰ₂  (λ₀)$ étant positive par hypothèse, on a
...
$α₁∈\mathbb{R}$ tels que, pour tout $\hat{u}∈U$
\begin{equation} ℰ_3 (λ₀ ; u₁, x₀, \hat{u}) +ℰ₂ (λ₀ ;
...
\begin{equation} ℰ₂ (λ₀ ; x₂, \hat{u}) + 2ℰ_3 (λ₀ ; u₁,
   x₁, \hat{u}) +ℰ_4  (λ₀ ; u₁, u₁, x₀, \hat{u})
   +ℰ_3 (λ₀ ; u₂, x₀, \hat{u}) + λ₂
   \dot{ℰ₂} (λ₀ ; x₀, \hat{u}) = α₂  \langle x₀,
   \hat{u} \rangle \end{equation}
soit, en prenant $\hat{u} = \widehat{v_i}∈V$ et en introduisant les
développements de $u₁$, $u₂$, $x₀ $ et $x₁$
...
\begin{equation} ℰ_3 (λ₀ ; v_i, v_j, w_k) = -ℰ₂ (λ₀ ;
