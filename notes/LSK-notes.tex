...
\begin{eqnarray}
  \Delta ℰ & = & ℰ (u_0 + \xi v + \eta w, λ_0)
  -ℰ (u_0, λ_0) \nonumber\\
  & = & \tfrac{1}{2} ℰ_2 (\xi v + \eta w, \xi v + \eta w) +
  \tfrac{1}{6} ℰ_3 (\xi v + \eta w, \xi v + \eta w, \xi v + \eta w)
  \nonumber\\
  &  &  + \tfrac{1}{24} ℰ_4 (\xi v + \eta w, \xi v + \eta
...
\begin{eqnarray}
  \Delta ℰ & = & \tfrac{1}{2} \eta^2 ℰ_2 (w, w) +
  \tfrac{1}{6} \xi^3 ℰ_3 (v, v, v) + \tfrac{1}{2} \xi^2 \eta
  ℰ_3 (v, v, w) \nonumber\\
  &  & + \tfrac{1}{2} \xi \eta^2 ℰ_3 (v, w, w) + \tfrac{1}{6}
  \eta^3 ℰ_3 (w, w, w) \nonumber\\
  &  & + \tfrac{1}{24} \xi^4 ℰ_4 (v, v, v, v) + \tfrac{1}{6} \xi^3
  \eta ℰ_4 (v, v, v, w) \nonumber\\
  &  & + \tfrac{1}{4} \xi^2 \eta^2 ℰ_4 (v, v, w, w) + \tfrac{1}{6}
  \xi \eta^3 ℰ_4 (v, w, w, w) \nonumber\\
  &  & + \tfrac{1}{24} \eta^4 ℰ_4 (w, w, w, w) +\mathcal{O} [(\xi^2
  + \eta^2)^2],
\end{eqnarray}
...
\begin{equation}
  \Delta ℰ= \tfrac{1}{2} \xi^4  \left[ ℰ_2 (w, w)
...
\begin{equation}
  ...
  + \xi_1^i \xi_1^j ℰ_3 (λ_0 ; v_i, v_j, \hat{w}) + 2
  λ_1 \xi_1^i  \dot{ℰ_2} (λ_0 ; v_i, \hat{w}) = 0,
\end{equation}
...
\begin{eqnarray}
  3 [ℰ_3 (λ_0 ; v_i, v_j, v_k) \xi_1^k + λ_1
  \dot{ℰ}_2 (λ_0 ; v_i, v_j)] \xi_2^j + 3 λ_2
  \dot{ℰ}_2 (λ_0 ; v_i, v_j) \xi_1^j &  &  \nonumber\\
...
\begin{eqnarray}
  ℰ_{, u  u} [u (\eta), λ (\eta) ; \hat{u}, \hat{v}] &
  = & ℰ_2 (λ_0 ; \hat{u}, \hat{v}) + \eta [ℰ_3
  (λ_0 ; u_1, \hat{u}, \hat{v})   + λ_1
  \dot{ℰ_2} (λ_0 ; \hat{u}, \hat{v})] \nonumber\\
  &  &  + \tfrac{1}{2} \eta^2  [ℰ_4 (λ_0 ; u_1, u_1,
...
\begin{eqnarray}
  ℰ_{, u  u} [u (\eta), λ (\eta) ; \hat{u}, \hat{u}] &
  = & ℰ_2 (λ_0 ; \hat{u}, \hat{u}) + \eta [ℰ_3
  (λ_0 ; u_1, \hat{u}, \hat{u}) + λ_1  \dot{ℰ}_2
  (λ_0 ; \hat{u}, \hat{u})] \nonumber\\
  &  & + \tfrac{1}{2} \eta^2  [ℰ_4 (λ_0 ; u_1, u_1, \hat{u},
...
\begin{eqnarray}
  ℰ_{, u  u} [u (\eta), λ (\eta) ; \hat{u}, \hat{u}] &
  = & \eta [ℰ_3 (λ_0 ; v_i, v_j, v_k) \xi_1^k + λ_1
  \dot{ℰ}_2 (λ_0 ; v_i, v_j)]  \hat{\xi}^i  \hat{\xi}^j
  \nonumber\\
  &  & + \tfrac{1}{2} \eta^2  [ℰ_4 (λ_0 ; v_i, v_j, v_k, v_l)
  \xi_1^k \xi_1^l +ℰ_3 (λ_0 ; v_i, v_j, v_k) \xi_2^k
   \nonumber\\
  &  & +ℰ_3 (λ_0 ; v_i, v_j, w_{k  l}) \xi_1^k
  \xi_1^l + λ_1 ℰ_3 (λ_0 ; v_i, v_j, w_k) \xi_1^k +
  λ_2  \dot{ℰ}_2 (λ_0 ; v_i, v_j) \nonumber\\
  &  &  + 2 λ_1  \dot{ℰ}_3 (λ_0 ; v_i, v_j,
  v_k) \xi_1^k + λ_1^2  \ddot{ℰ}_2 (λ_0 ; v_i, v_j)]
...
u_1$ ($\hat{\xi}^i = \xi_1^i$)
\begin{equation}
  ℰ_{, u  u} [u (\eta), λ (\eta) ; u_1, u_1] = -
  λ_1 \eta \dot{ℰ}_2 (λ_0 ; u_1, u_1) + o (\eta) .
\end{equation}
...
\begin{equation}
  λ = λ_0 - \frac{\xi ℰ_{1  1  1}
  (λ_0)}{2 \dot{ℰ}_{1  1} (λ_0)} + o (\xi),
...
s'écrit
\begin{equation}
  ℰ_{, u  u} [u (\eta), λ (\eta) ; v_1, v_1] = \eta
  [ℰ_{1  1  1} (λ_0) + λ_1
  \dot{ℰ}_{1  1} (λ_0)] + o (\eta),
\end{equation}
soit, en substituant l'équation~\eqref{eq20220203144712}
\begin{equation}
  ℰ_{, u  u} [u (\eta), λ (\eta) ; v_1, v_1] = -
  λ_1 \eta \dot{ℰ}_{1  1} (λ_0) + o (\eta) .
\end{equation}
...
\begin{eqnarray}
  ℰ_{, u  u} [u (\eta), λ (\eta) ; v_1, v_1] & = &
  \tfrac{1}{2} \eta^2  [ℰ_{1  1  1  1}
  (λ_0) + 2ℰ_3 (λ_0 ; v_1, v_1, u_2) + 2 λ_2
  \dot{ℰ}_{1  1} (λ_0)] + o (\eta^2) \nonumber\\
  & = & \tfrac{5}{12} \eta^2 ℰ_{1  1  1  1}
  (λ_0) + o (\eta^2) .
\end{eqnarray}
...
avec
\begin{equation}
  \label{eq20220107054629} f (\eta) =ℰ_{, u} [u (\eta), λ
  (\eta) ; \hat{u}] \quad \text{et} \quad F (\eta, λ) =ℰ_{, u}
...
\begin{equation}
  \partial_{\eta} F (\eta, λ) =ℰ_{, u  u} [u^{\ast}
  (λ) + U (\eta), λ ; U' (\eta), \hat{u}],
\end{equation}
\begin{eqnarray}
  \partial_{\eta  \eta}^2 F (\eta, λ) & = & ℰ_{, u
...
soit, en $\eta = 0$
\begin{equation}
  \partial_{\eta} F (0, λ) =ℰ_2 (λ ; u_1, \hat{u}),
\end{equation}
\begin{equation}
  \partial_{\eta  \eta}^2 F (0, λ) =ℰ_3 (λ ;
  u_1, u_1, \hat{u}) +ℰ_2 (λ ; u_2, \hat{u}),
\end{equation}
\begin{equation}
  \partial_{\eta  \eta  \eta}^3 F (0, λ) =ℰ_4
...
fondamentale, soit
\begin{equation} F (\eta, λ) =ℰ [u^{\ast} (λ) + U (\eta), λ]
   -ℰ [u^{\ast} (λ), λ] \end{equation}
et
\begin{equation} f (\eta) = F [\eta, λ_0 + λ (\eta)] . \end{equation}
...
\begin{equation} \partial_{\eta} F (\eta, λ) =ℰ_{, u} [u^{\ast} (λ) +
   U (\eta), λ ; U' (\eta)], \end{equation}
\begin{equation} \partial_{\eta  \eta}^2 F (\eta, λ) =ℰ_{, u
    u} [u^{\ast} (λ) + U (\eta), λ ; U' (\eta), U' (\eta)]
   +ℰ_{, u} [u^{\ast} (λ) + U (\eta), λ ; U'' (\eta)],
\end{equation}
\begin{eqnarray*}
  \partial_{\eta  \eta  \eta}^3 F (\eta, λ) & = &
  ℰ_{, u  u  u} [u^{\ast} (λ) + U (\eta),
  λ ; U' (\eta), U' (\eta), U' (\eta)]\\
  &  &    + 3ℰ_{, u  u}
...
(0)$
\begin{eqnarray*}
  f'''' (0) & = & ℰ_4 (λ_0 ; v_i, v_j, v_k {, v_l} ) \xi_1^i
  \xi_1^j \xi_1^k \xi_1^l + 6ℰ_3 (λ_0 ; v_i, v_j, v_k) \xi_1^i
  \xi_1^j \xi_2^k\\
  &  &  + 6ℰ_3 (λ_0 ; v_i, v_j, w_{k  l})
  \xi_1^i \xi_1^j \xi_1^k \xi_1^l + 12 λ_1  \dot{ℰ}_2
  (λ_0 ; v_{i }, w_{j  k}) \xi_1^i \xi_1^j \xi_1^k\\
  &  &  - 3ℰ_3 (λ_0 ; v_i, v_j, w_{k  l})
  \xi_1^i \xi_1^j \xi_1^k \xi_1^l - 12 λ_1  \dot{ℰ}_2
  (λ_0 ; v_i, w_{j  k}) \xi_1^i \xi_1^j \xi_1^k\\
  &  &  - 3 λ_1^2  [\dot{ℰ}_2 (λ_0 ; v_i, w_j)
  + \dot{ℰ}_2 (λ_0 ; v_j, w_i)] \xi_1^i \xi_1^j + 4 λ_1
  \dot{ℰ}_3 (λ_0 ; v_i, v_j, v_k) \xi_1^i \xi_1^j \xi_1^k\\
  &  &  + 12 λ_1  \dot{ℰ}_2 (λ_0 ; v_i,
  v_j) \xi_1^i \xi_2^j + 12 λ_1  \dot{ℰ}_2 (λ_0 ;
  v_i, w_{j  k}) \xi_1^i \xi_1^j \xi_1^k\\
  &  &  + 6 λ_1^2  [\dot{ℰ}_2 (λ_0 ; v_i,
  w_j) + \dot{ℰ}_2 (λ_0 ; v_j, w_i)] \xi_1^i \xi_1^j + 6
  λ_1^2  \ddot{ℰ}_2 (λ_0 ; v_i, v_j) \xi_1^i \xi_1^j\\
  &  &  + 6 λ_2  \dot{ℰ}_2 (λ_0 ; v_i, v_j)
  \xi_1^i \xi_1^j\\
  & = & \left[ ℰ_4 (λ_0 ; v_i, v_j, v_k {, v_l} ) +
  3ℰ_3 (λ_0 ; v_i, v_j, w_{k  l}) \right] \xi_1^i
  \xi_1^j \xi_1^k \xi_1^l\\
  &  &  + 4 λ_1  [\dot{ℰ}_3 (λ_0 ; v_i, v_j,
  v_k) + 3 \dot{ℰ}_2 (λ_0 ; v_i, w_{j  k})] \xi_1^i
  \xi_1^j \xi_1^k\\
  &  &  + \{ 3 λ_1^2  [\dot{2 \ddot{ℰ}_2
  (λ_0 ; v_i, v_j) + \dot{ℰ}}_2 (λ_0 ; v_i, w_j) +
  \dot{ℰ}_2 (λ_0 ; v_j, w_i)] + 6 λ_2
  \dot{ℰ}_2 (λ_0 ; v_i, v_j) \} \xi_1^i \xi_1^j\\
  &  &  + 6ℰ_3 (λ_0 ; v_i, v_j, v_k) \xi_1^i \xi_1^j
  \xi_2^k + 12 λ_1  \dot{ℰ}_2 (λ_0 ; v_i, v_j)
  \xi_1^i \xi_2^j\\
...
λ_0 + λ (\eta)]$, avec
\begin{equation} F (\eta, λ) =ℰ_{, u  u} [u^{\ast} (λ) + U
   (\eta), λ ; \hat{u}, \hat{v}] . \end{equation}
...
\begin{equation} \partial_{\eta} F (\eta, λ) =ℰ_{, u  u  u}
   [u^{\ast} (λ) + U (\eta), λ ; U' (\eta), \hat{u}, \hat{v}], \end{equation}
\begin{eqnarray}
  \partial_{\eta  \eta}^2 F (\eta, λ) & = & ℰ_{, u
...
\end{eqnarray}
soit, en $\eta = 0$
\begin{equation} \partial_{\eta} F (0, λ) =ℰ_3 (λ ; u_1, \hat{u},
   \hat{v}) \text{} \end{equation}
et
\begin{equation} \partial_{\eta  \eta}^2 F (0, λ) =ℰ_4 (λ ;
   u_1, u_1, \hat{u}, \hat{v}) +ℰ_3 (λ ; u_2, \hat{u},
   \hat{v}), \end{equation}
et en dérivant cette fois par rapport à $λ$
\begin{equation} \partial_{\eta  λ}^2 F (0, λ) = \dot{ℰ_3}
...
\begin{eqnarray*}
  ℰ_{, u  u} [u (\eta), λ (\eta) ; x, \hat{u}] & = &
  ℰ_2 (λ_0 ; x_0, \hat{u}) + \eta ℰ_2 (λ_0 ;
  x_1, \hat{u}) + \tfrac{1}{2} \eta^2 ℰ_2 (λ_0 ; x_2,
  \hat{u})\\
  &  & + \eta ℰ_3 (λ_0 ; u_1, x_0, \hat{u}) + \eta^2
  ℰ_3 (λ_0 ; u_1, x_1, \hat{u})\\
  &  & + \eta λ_1  \dot{ℰ_2} (λ_0 ; x_0, \hat{u}) +
  \eta^2 λ_1  \dot{ℰ_2} (λ_0 ; x_1, \hat{u})\\
  &  & + \tfrac{1}{2} \eta^2  [ℰ_4 (λ_0 ; u_1, u_1, x_0,
...
base $v_i$
\begin{equation} ℰ_3 (λ_0 ; v_j, v_k, \hat{u}) \xi_1^k \chi_0^j
...
\begin{equation} [ℰ_3 (λ_0 ; v_i, v_j, v_k) \xi_1^k + λ_1
   \dot{ℰ_2} (λ_0 ; v_i, v_j)] \chi_0^j = \alpha_1 \chi_0^i,
\end{equation}
soit encore
\begin{equation} [E_{i  j  k} (λ_0) \xi_1^k + λ_1 F_{i
...
\begin{equation} ℰ_3 (λ_0 ; v_j, v_k, \hat{w}) \xi_1^k \chi_0^j
...
\begin{equation} y_1 = \xi_1^i \chi_0^j w_{i  j} + λ_1 \chi_0^i w_i, \end{equation}
soit
\begin{equation} x_1 = \chi_1^i v_i + \xi_1^i \chi_0^j w_{i  j} + λ_1 \chi_0^i
   w_i . \end{equation}
...
\begin{equation} ℰ_2 (λ_0 ; x_2, \hat{u}) + 2ℰ_3 (λ_0 ; u_1,
   ...
   \dot{ℰ_2} (λ_0 ; x_0, \hat{u}) = \alpha_2  \langle x_0,
   \hat{u} \rangle \end{equation}
soit, en prenant $\hat{u} = \widehat{v_i}∈V$ et en introduisant les
développements de $u_1$, $u_2$, $x_0 $ et $x_1$
\begin{equation} ℰ_4  (λ_0 ; v_i, v_j, v_k, v_l) \chi_0^j \xi_{1 }^k \xi_1^l
...
\begin{equation} ℰ_3 (λ_0 ; v_i, v_j, w_{k  l}) \xi_1^j \xi_1^k
   \xi_1^l = \text{} \tfrac{1}{3}  [ℰ_3 (λ_0 ; v_i, v_j, w_{k
    l})  +ℰ_3 (λ_0 ; v_i, v_k, w_{j
   l}) +ℰ_3 (λ_0 ; v_i, v_l, w_{j  k}] \xi_1^j \xi_1^k
   \xi_1^l . \end{equation}
...
\begin{equation} ℰ_3 (λ_0 ; v_i, v_j, w_k) \xi_1^j \xi_1^k = \tfrac{1}{2}
   [ℰ_3 (λ_0 ; v_i, v_j, w_k) +ℰ_3 (λ_0 ; v_i,
   w_j, v_k)] \xi_1^j \xi_1^k . \end{equation}
...
soit finalement
\begin{equation} ℰ_3 (λ_0 ; v_i, v_j, w_k) \xi_1^j \xi_1^k =
   [\dot{ℰ}_2 (λ_0 ; v_j, w_{i  k}) +
   \dot{ℰ}_2 (λ_0 ; v_k, w_{i  j})] \xi_1^j \xi_1^k .
\end{equation}
