...
If \((E_{ijk} ξ₁^k + λ₁ F_{ij})\) is \emph{positive (resp. negative) definite},
then \(α₁ > 0\) (resp. \(α₁ < 0\)) and \(α\) has the sign of \(η\)
(resp. \(-η\)). The bifurcated branch is \emph{stable} (resp. \emph{unstable})
for \(η > 0\), \emph{unstable} (resp. \emph{stable}) for \(η < 0\). In both
cases, there is a stability switch at the critical load. Owing to our previous
result, we therefore conclude in both cases that the bifurcated branch is
\emph{unstable below the critical load} and \emph{stable above the critical
  load}. \pdfmargincomment{Cela me semble être en contradiction avec le
  graphique de la Fig 2.4.7}

If \((E_{ijk} ξ₁^k + λ₁ F_{ij})\) is \emph{undefined}, we cannot conclude.
