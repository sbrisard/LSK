...
\end{theorem}
\begin{proof}
  Soient \(u, v∈\kerℬ\), \(\alpha∈ℝ\) et \(w=u+\alpha
  v\). Montrons que \(w∈\kerℬ\). Il suffit d'évaluer
  \(ℬ(w, w)\)
  \begin{equation}
    ℬ(w, w)=ℬ(u+\alpha v, u+\alpha v)=ℬ(u, u)+2\alphaℬ(u, v)+\alpha²ℬ(v, v),
  \end{equation}
  ...
  \(ℬ(w, w)=2\alphaℬ(u, v)\). La forme bilinéaire étant
  positive, cette grandeur est positive, \emph{quelle que soit la valeur de
    \(\alpha∈ℝ\)}. On en déduit donc que \(ℬ(u, v)=0\), puis
  que \(ℬ(w, w)=0\).
\end{proof}
...
\end{equation}
\begin{proof}
  Soient \(u∈\kerℬ\), \(v∈ V\) et \(\alpha∈ℝ\). Comme
  précédemment, on écrit que \(ℬ(w, w)≥0\), avec \(w=\alpha u+v\)
  \begin{equation}
    ℬ(w, w)=2\alphaℬ(u, v)+ℬ(v, v)≥0,
  \end{equation}
  où l'on a tenu compte de ce que \(ℬ(u, u)=0\). L'expression
  précédente, affine en \(\alpha\), a un signe constant. Le terme linéaire en
  \(\alpha\) est donc nul, soit \(ℬ(u, v)=0\).
...
\begin{equation}
  ...
  \begin{aligned}[b]
    h''(η)={}&\partial_{ηη}² H(η, λ^*+λ)
    +2λ'\partial_{ηλ}² H(η, λ^*+λ)\\
    &+λ''\partial_{λ}H(η, λ^*+λ)
    +λ'^2\partial_{λλ}²H(η, λ^*+λ).
  \end{aligned}
\end{equation}
...
\begin{equation}
  \begin{aligned}[b]
    h''(0)={}&𝒬^*(u₁, u₁, \hat{u}, \hat{v})
    +2λ₁\dot{𝒯}^*(u₁, \hat{u}, \hat{v})
    +λ₁²\ddot{ℋ}^*(\hat{u}, \hat{v})\\
    &+2𝒯^*(u₂, \hat{u}, \hat{v})
    +2λ₂\dot{ℋ}^*(\hat{u}, \hat{v}).
  \end{aligned}
\end{equation}
