...
dans un état $u_0 + ξ v + \eta w$ voisin du point d'équilibre $u_0$,
avec $ξ, \eta∈\mathbb{R}$ {\guillemotleft} petits {\guillemotright}, $v
\in V$ and $w∈W$. On obtient alors, à l'ordre 4 en $ξ$ et $\eta$
\begin{eqnarray}
  \Delta ℰ & = & ℰ (u_0 + ξ v + \eta w, λ_0)
  -ℰ (u_0, λ_0) \nonumber\\
  & = & \tfrac{1}{2} ℰ_2 (ξ v + \eta w, ξ v + \eta w) +
  \tfrac{1}{6} ℰ_3 (ξ v + \eta w, ξ v + \eta w, ξ v + \eta w)
  \nonumber\\
  &  &  + \tfrac{1}{24} ℰ_4 (ξ v + \eta w, ξ v + \eta
  w, ξ v + \eta w, ξ v + \eta w) +\mathcal{O} [(ξ^2 + \eta^2)^2],
\end{eqnarray}
...
\begin{eqnarray}
  ...
  \tfrac{1}{6} ξ^3 ℰ_3 (v, v, v) + \tfrac{1}{2} ξ^2 \eta
  ℰ_3 (v, v, w) \nonumber\\
  &  & + \tfrac{1}{2} ξ \eta^2 ℰ_3 (v, w, w) + \tfrac{1}{6}
  \eta^3 ℰ_3 (w, w, w) \nonumber\\
  &  & + \tfrac{1}{24} ξ^4 ℰ_4 (v, v, v, v) + \tfrac{1}{6} ξ^3
  \eta ℰ_4 (v, v, v, w) \nonumber\\
  &  & + \tfrac{1}{4} ξ^2 \eta^2 ℰ_4 (v, v, w, w) + \tfrac{1}{6}
  ξ \eta^3 ℰ_4 (v, w, w, w) \nonumber\\
  &  & + \tfrac{1}{24} \eta^4 ℰ_4 (w, w, w, w) +\mathcal{O} [(ξ^2
  + \eta^2)^2],
\end{eqnarray}
...
positive ou nulle pour tous $ξ$ et $\eta$ suffisamment petits. En prenant
...
\begin{eqnarray}
  ℰ_{, u  u} [u (\eta), λ (\eta) ; \hat{u}, \hat{u}] &
  = & \eta [ℰ_3 (λ_0 ; v_i, v_j, v_k) ξ_1^k + λ_1
