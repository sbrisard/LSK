...
\end{equation}
At \(ξ^i = 0\), the above equations read and
\begin{multline}
...
differentiated \wrt{} \(ξ^j\)
\begin{multline}
  ℰ_{,uuu}(u^\ast + ξ^k v_k + w, λ; v_i + w_{,i}, v_j + w_{,j}, \hat{w}) + ℰ_{,uu}(u^\ast + ξ^k v_k + w, λ; w_{,ij}, \hat{w}) = 0\\
  \text{for all }\hat{w} ∈ W,
\end{multline}
and, at \(ξ^1 = \cdots = ξ^m = 0, λ = λ₀\)
\begin{equation}
  ℰ_3(λ₀; v_i, v_j, \hat{w}) + ℰ₂(λ₀; w_{,ij}, \hat{w}) = 0 \quad \text{for all } \hat{w} ∈ W,
\end{equation}
where it is recalled that \(w_{,i} = w_{,j} = 0\). The variational
problem~\eqref{eq:20220519164523} is recognized, and we get \(w_{,ij} = w_{ij}\)
at \(ξ^1 = \cdots = ξ^m = 0, λ = λ₀\).
